...
(function () {
  ...
  // ── Мои площадки ───────────────────────────────────────────────────────────
  // Ритм («14 за 30 дней») — главное число: план должен быть выполнимым, а не
  // желаемым. «Не замерено» — честный ответ, а не ноль: Instagram и TikTok без
  // входа не читаются (решения контент-машины, перенесены как есть).
  function экранПлощадок() {
    var к = (Д.экраны && Д.экраны.channels) || [];
    if (!к.length) {
      return шапка('Мои площадки', 'Где мы публикуемся и с каким ритмом.')
        + пусто('Площадок пока нет',
            'Они появятся после прогона исследования с заполненными соцсетями в брифе: '
            + 'там мы смотрим, что клиент уже выкладывает и как это живёт. '
            + 'Заполните соцсети в брифе и запустите контент-радар.')
        + приписка();
    }
    var всего = к.reduce(function (а, х) { return а + (Number(х.planned) || 0); }, 0);
    var карточки = к.map(function (х) {
      var число = х.followers == null || х.followers === '' ? 'не замерено'
        : String(х.followers).replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
      return '<div class="card tcard">'
        + '<div class="trub">' + esc(х.platform || 'площадка') + '</div>'
        + '<h3>' + esc(х.title || х.url || 'канал') + '</h3>'
        + '<div class="tfmts"><span>ритм</span>'
        + '<span class="tfmt">' + (х.planned ? х.planned + ' за 30 дней' : 'ритм не задан') + '</span>'
        + '<span class="tfmt">подписчиков: ' + esc(число) + '</span></div>'
        + (х.note ? '<p class="note" style="margin:10px 0 0">' + esc(х.note) + '</p>' : '')
        + '</div>';
    }).join('');
    return шапка('Мои площадки', 'Где мы публикуемся и с каким ритмом.',
        [['Площадок', к.length], ['Материалов за 30 дней', всего || '']])
      + карточки
      + '<p class="note">«Не замерено» — честный ответ, а не ноль: часть площадок '
      + 'без входа не читается, и подставить туда ноль значило бы занизить охват.</p>'
      + приписка();
  }

  var ЭКРАНЫ = {
    channels_own: экранПлощадок,
