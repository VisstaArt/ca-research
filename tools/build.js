...
// Метка версии исследования внутри platform.js. Оболочка грузит инструмент в
// РАМКЕ по адресу index.html?…, и без метки браузер отдавал рамке старую
// копию: владелица обновляла страницу, перезапускала модуль и видела прежний
// отчёт (17.09). Метка меняется вместе с app.js — рамка обязана скачать новое.
(function () {
  var app = rd(BASE + 'app.js');
  var pl = rd(BASE + 'platform.js');
  if (!app || !pl) return;
  var fp = fingerprint(app).slice(0, 8);
  var next = pl.replace(/var ВЕРСИЯ_ИССЛЕДОВАНИЯ = '[^']*';/, "var ВЕРСИЯ_ИССЛЕДОВАНИЯ = '" + fp + "';");
  if (next !== pl) {
    wr(BASE + 'platform.js', next);
    console.log('  platform.js: версия рамки исследования → ' + fp);
  }
})();
