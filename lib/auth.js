...
(function (root) {
  ...
  // Регистрация. Нужна, чтобы путь можно было пройти с чистого листа: завести
  // почту и пароль, войти и увидеть пустую платформу, а не чужие данные.
  // Возвращает {ok} либо {ok:false, error} — человеку надо показать ПРИЧИНУ:
  // «почта занята» и «пароль короткий» требуют разных действий, а общее
  // «не получилось» заставляет гадать.
  //
  // Если в проекте включено подтверждение почты, Supabase не вернёт токены
  // сразу — тогда честно говорим, что ждём письмо, а не делаем вид, что вошли.
  async function signUp(email, password) {
    try {
      ...
      if (!res.ok) {
        ...
        return { ok: false, error: msg || 'Не удалось зарегистрировать' };
      }
      if (d.access_token) { setTokens(d.access_token, d.refresh_token); return { ok: true, signedIn: true }; }
      return { ok: true, signedIn: false, confirm: true };
  ...
  // ПОТОЛОК ОЖИДАНИЯ. Без него зависший запрос держит весь прогон: браузер
  // ждёт ответа бесконечно, а у нас всё идёт по очереди — один молчащий
  // сервер останавливает модуль целиком (владелица 17.09: «идёт тридцать пять
  // минут и не заканчивается»). Три минуты хватает даже медленной модели;
  // всё, что дольше, — уже не ответ, а зависание, и лучше повторить.
  const ПОТОЛОК_МС = 180000;
  async function сПотолком(url, opts, мс) {
    if (typeof AbortController === 'undefined') return fetch(url, opts);
    const ctrl = new AbortController();
    const таймер = setTimeout(() => ctrl.abort(), мс || ПОТОЛОК_МС);
    try {
  ...
  async function authFetch(url, opts) {
    ...
    const attempt = async () => {
      let res = await сПотолком(url, { ...o, headers: withAuth(o.headers) }, o.таймаутМс);
      if (res.status === 401) {
        if (await refreshTokens()) res = await сПотолком(url, { ...o, headers: withAuth(o.headers) }, o.таймаутМс);
      }
      return res;
    };
  ...
  root.CAAuth = {
    ...
    signIn: signIn, signUp: signUp, refreshTokens: refreshTokens, authFetch: authFetch,
  };
