...
// Память браузера имеет предел, и раньше запись падала МОЛЧА: человек узнавал
// об этом, когда после обновления страницы оплаченного прогона не оказывалось
// (владелица 17.09).
[['запись сообщает, удалась ли она', /localStorage\.setItem\(SK, JSON\.stringify\(l\)\); return true;/],
 ['при переполнении сбрасываются материалы, а не результат', /собранные материалы сброшены, результаты сохранены/],
 ['человеку говорят о переполнении', /Память браузера переполнена, и результат туда не/],
 ['материалы не уходят в облако', /body: JSON\.stringify\(безМатериалов\(p\)\)/]].forEach(function(п){
  if (п[1].test(исх)) console.log('  ok   '+п[0]);
  else { провалов++; console.log('  FAIL '+п[0]); }
});
console.log(провалов===0 ? '\nвсё сошлось (хранение результата)' : '\nПРОВАЛОВ: '+провалов);

// Версия загруженного кода видна ВСЕГДА, а не только в карточке результата:
// до первого прогона карточки нет, и проверить свежесть страницы было нечем
// (владелица 17.09: «обновила, а метки кода не показывает»).
if (/код \{отпечатокСборки\(\)\}/.test(исх))
  console.log('  ok   версия кода видна на экране прогона');
else { провалов++; console.log('  FAIL версию загруженного кода не видно'); }
console.log(провалов===0 ? '\nвсё сошлось (версия на виду)' : '\nПРОВАЛОВ: '+провалов);

// Черновик — незаконченный модуль, и выглядеть он должен именно так: у него
// ещё копятся счётчики, и метка «без поиска» пугала бы зря (владелица 17.09).
[['черновик подписан как незаконченный', /if \(r\.черновик\) return \(/],
 ['в черновик кладутся счётчики поиска', /\.\.\.\(searchCallCount \? \{ searchCalls: searchCallCount \} : \{\}\)/]]
 .forEach(function(п){
   if (п[1].test(исх)) console.log('  ok   '+п[0]);
   else { провалов++; console.log('  FAIL '+п[0]); }
 });
console.log(провалов===0 ? '\nвсё сошлось (черновик)' : '\nПРОВАЛОВ: '+провалов);
