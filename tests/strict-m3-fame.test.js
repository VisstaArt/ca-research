...
var д={конкуренты:[
 ...
 {название:'Тихий Бренд',сайт:'tihiy.ru',известность:'не замерено',чем_известен:'попапы',
  цена:{от:900,до:null,валюта:'RUB',период:'в месяц',как_узнали:'тарифы'}},
...
замерИзвестностиСтрогое(д,{geoMarket:'Россия'}).then(function(р){
  ...
  ok('лишних обращений нет', вызовов, 4);
  ...
  // Почему в кольце меньше имён, чем в карте рынка (владелица 17.09: «выпало
  // мало конкурентов, всего 4»). Отчёт обязан сказать это сам.
  ok('видно, сколько замерено из скольких', /Замерено названий: <b>3 из 5<\/b>/.test(html), true);
  ok('названы те, по кому числа нет', /Тихий Бренд/.test(html), true);
  ok('сказано, что в карте рынка они остаются', /в карте рынка они остаются/.test(html), true);
