...
// Служебный код модели: метка блока и блок данных графика. В строгом пути
// вид задаём мы, но модель по привычке дописывает их в текстовые поля — и
// они выпадают в отчёт куском кода (владелица 17.09, отчёт M3).
var сКодом=безСлужебныхМеток({итог:{что_узнали:[
  '@@BLOCK 06\nРынок поделён между тремя игроками.',
  'Позиция на графике — оценка по брифу. <<<CHART_DATA_START>>> {"competitorMatrix":[{"название":"Мы","price":3}]} <<<CHART_DATA_END>>>',
  'Оборванный блок: <<<CHART_DATA_START>>> {"a":1',
  'Собаки @@ посреди строки.']}});
ok('метка блока убрана', /@@/.test(сКодом.итог.что_узнали[0]), false);
ok('текст после метки остался', /Рынок поделён/.test(сКодом.итог.что_узнали[0]), true);
ok('блок данных графика вырезан', /CHART_DATA|competitorMatrix/.test(сКодом.итог.что_узнали[1]), false);
ok('пояснение перед блоком осталось', /оценка по брифу/.test(сКодом.итог.что_узнали[1]), true);
ok('оборванный блок тоже вырезан', /CHART_DATA/.test(сКодом.итог.что_узнали[2]), false);
ok('одиночные «@@» убраны', /@@/.test(сКодом.итог.что_узнали[3]), false);
