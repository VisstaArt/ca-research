...
check('шкала показана один раз', (разметка.match(/class="note confleg"/g)||[]).length === 1);
check('цифры в шкале выделены', /<b>5<\/b> — три и более независимых свежих источника/.test(разметка));
check('шкала стоит после таблицы с колонкой',
  разметка.indexOf('confleg') > разметка.indexOf('rpt-nprof'));
