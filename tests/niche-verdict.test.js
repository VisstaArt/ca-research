...
// Прямое «не идём» от разведки сильнее баллов: только у таких ниш печатается
// разбор «почему не идём», и владелица 15.09 просила их сохранять —
// «мы их проверили, и видно почему». Внизу списка, но на виду.
var сОтказом = [
  {name:'Стройка', score:18, отказ:true},
  {name:'Школы',   score:18, отказ:false},
  {name:'Клиники', score:12, отказ:false},
].map(function(н){ return {name:н.name, score:н.score,
  verdict: н.отказ ? 'не идём' : вердиктПоБаллам(н.score)}; })
 .sort(function(a,b){ return (весВердикта(a.verdict)-весВердикта(b.verdict)) || (b.score-a.score); });
check('отказ разведки сильнее высоких баллов', сОтказом[2].name, 'Стройка');
check('отброшенная ниша не пропала', сОтказом.length, 3);
check('порядок остальных по баллам', сОтказом.slice(0,2).map(function(x){return x.name;}), ['Школы','Клиники']);
