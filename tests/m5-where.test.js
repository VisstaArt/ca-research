...
ok('значок по площадке — узнаваемый', [D[0][2], D[1][2]], ['forum','telegram']);
...
// Не нашли себя в таблице — вписываем себя сами: карта без «нас» бесполезна.
var P2=точки('Совсем другая компания');
ok('нас нет в таблице — добавляем строкой', P2.filter(function(x){return x[4];}).map(function(x){return x[0];}),
   ['Совсем другая компания']);
ok('чужую строку при этом не помечаем', P2.filter(function(x){return x[4];}).length, 1);
