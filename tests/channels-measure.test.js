...
eval(взять('площадкаПоАдресу')); eval(взять('ссылкиНаПлощадки'));
eval(взять('замерКанала')); eval(взять('замерКаналовСтрогое'));
...
// Запятая значит РАЗНОЕ: при множителе она десятичная, без него — разделитель
// тысяч. 18.09 канал на «1,71 тыс.» подписчиков приехал как 171 000 — ошибка
// в сто раз, и она попала в сводку форматов как медиана охвата.
ok('дробь с множителем — не тысячи', числоИзТекста('1,71 тыс.'), 1710);
ok('тысячи словом', числоИзТекста('1,08 тысячи'), 1080);
ok('запятая без множителя — разделитель тысяч', числоИзТекста('171,710'), 171710);
ok('круглые тысячи', числоИзТекста('400 тыс.'), 400000);
...
замерКанала('https://t.me/good').then(function(з){
  ...
  ok('замер перебил «не замерено»', /12 345 подписчиков — наш замер/.test(р.каналы[0].подписчики), true);
...
(function(){
  ...
  var витрина=пост(11,'Как вернуть уходящего посетителя <b>попапом</b>',1000,10,'2026-09-01')
            +пост(12,'Дайджест новостей рынка',5000,5,'2026-09-05');
  var прежний2=globalThis.fetch;
  // Витрина отдаёт страницами: продолжение через ?before=<номер>. Вторая
  // страница нужна, иначе примеров в отчёт попадает три-четыре на всю нишу
  // (владелица 17.09).
  var витрина2=пост(7,'Старый пост про формы',300,9,'2026-08-20');
  globalThis.fetch=function(u){
    var адрес=decodeURIComponent(String(u).split('url=')[1]||'');
    if (адрес==='https://t.me/s/envybox')
      return Promise.resolve({ok:true,text:function(){return Promise.resolve(витрина);}});
    if (адрес==='https://t.me/s/envybox?before=11')
      return Promise.resolve({ok:true,text:function(){return Promise.resolve(витрина2);}});
    return прежний2(u);
  };
  постыКанала('https://t.me/envybox','Envybox').then(function(п){
    ок('посты сняты со всех страниц витрины', п.length===3, JSON.stringify(п.map(function(x){return x.url;})));
    ок('со второй страницы пост тоже взят',
       п.some(function(x){return /envybox\/7$/.test(x.url);}), JSON.stringify(п.map(function(x){return x.url;})));
    ок('разметка вычищена из текста', п.every(function(x){return x.текст.indexOf('<')<0;}), п[0].текст);
    ок('первым идёт пост с наибольшей долей отклика', п[0].доля===3, JSON.stringify(п[0]));
    var первыйСоСтраницы=п.filter(function(x){return /envybox\/11$/.test(x.url);})[0]||{};
    ок('просмотры сняты', первыйСоСтраницы.просмотры===1000, String(первыйСоСтраницы.просмотры));
    ок('реакции сняты', первыйСоСтраницы.реакции===10, String(первыйСоСтраницы.реакции));
    ок('адрес поста собран', первыйСоСтраницы.url==='https://t.me/envybox/11', первыйСоСтраницы.url);
    ок('имя конкурента при посте', первыйСоСтраницы.чей==='Envybox', первыйСоСтраницы.чей);
    ок('дата сохранена', первыйСоСтраницы.дата==='2026-09-01', первыйСоСтраницы.дата);
    return материалыКаналовКонкурентов([{url:'https://t.me/envybox',конкурент:'Envybox'}]);
  }).then(function(в){
    ок('сборка по всем каналам работает', в.посты.length===3, String(в.посты.length));
...
(function(){
  ...
  замерКаналовСтрогое(д,{}).then(function(р){
    ...
    ок('подписчики попали в строку', /3\s?960 подписчиков — наш замер/.test(String(к.подписчики||'')), String(к.подписчики));
    // ── СВЯЗКИ ПЛОЩАДОК. Владелица 19.09: «блогер тянет из ютуба в телеграм;
// телеграм сам себя не пиарит». Ссылки на соседние площадки автора лежат на
// уже скачанной странице — собираем их и считаем переходы «откуда → куда».
(function(){
  var html = '<html><head>'
    + '<meta property="og:title" content="Канал про магазины">'
    + '<meta property="og:description" content="Разборы для интернет-магазинов">'
    + '</head><body>Наш ютуб: https://youtube.com/@shopguru и чат '
    + 'https://t.me/shopchat, а это свой же https://t.me/s/shopguru</body></html>';
  var в = ссылкиНаПлощадки(html, 'https://t.me/s/shopguru');
  ок('ссылка на соседнюю площадку найдена',
     в.some(function(x){return x.площадка==='YouTube';}),
     'переход с телеграма на видео не виден');
  ок('свою же площадку в связки не берём',
     !в.some(function(x){return x.площадка==='Telegram';}),
     'канал ссылается сам на себя — это не воронка');
  ок('витрину t.me/s/ не считаем ссылкой',
     !в.some(function(x){return /t\.me\/s\//.test(x.url);}),
     'служебный адрес попал в связки');
  ок('связка доезжает до замеренного канала',
     /ведёт: з\.ведёт \|\| \[\]/.test(SRC),
     'ссылки собраны, но до сводки не доехали');
  ок('связки считаются парами площадок, без направления',
     /const ключ = \[своя, п\]\.sort\(\)\.join\(' \+ '\);/.test(SRC)
     && /из\.переходы = \[\.\.\.пары\.values\(\)\]/.test(SRC),
     'воронку ниши не из чего собрать');
})();
