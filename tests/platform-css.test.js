...
// Метка версии в ссылке законна: сборка штампует её от содержимого файла,
// чтобы кэш браузера не прятал обновления («всё без изменений» 14.09).
check('живая часть отдельным файлом', /src="platform\.js(\?v=[0-9a-f]+)?"/.test(SHELL), true);
