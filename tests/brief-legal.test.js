...
ок('у физлица предупреждение появляется', /нужны политика и согласие/.test(уФизлица), 'молчит');
...
ок('это названо предупреждением, а не отказом', /предупреждение, а не отказ/.test(уФизлица), 'нет оговорки');
ок('вторая часть есть, а форма не указана — предупреждение тоже есть',
   /нужны политика и согласие/.test(правилоБезРеквизитов({funnel_needed:'да',legal_form:''})), 'молчит');
