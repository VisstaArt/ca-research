...
// Ссылка на картинку слайда подписывается токеном вошедшего, а не сервисным
// ключом: с миграции 004 владелец читает свои файлы по правилу базы.
ok('слайды подписываются токеном пользователя',
   /object\/sign\/content/.test(src) && /Bearer ' \+ A\.getAccessToken\(\)/.test(src), true);
ok('ссылка живёт час', /expiresIn: 3600/.test(src), true);
