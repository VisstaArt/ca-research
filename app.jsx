...
const { GLOBAL_MODS, isPerNiche, dropOrphans, nichesOf, resKey,
        splitMdRow, isMdSeparator, parseMdTables, buildModuleEntry,
        pickTable, pickColumn, withStableIds } = CAContract;
// Название модуля берётся из MODULES — это конфиг ИНТЕРФЕЙСА, и сборщик
// контракта про него знать не должен. Поэтому отдаём ему функцию поиска.
const moduleTitleOf = id => { const m = MODULES.find(x => x.id === id); return m ? (m.titleRu || m.title) : id; };
const buildAgentPackage = (brief, lang, results, report) =>
  CAContract.buildAgentPackage(brief, lang, results, report, { titleOf: moduleTitleOf });
...
// Лимит токенов в минуту у счёта владелицы: 30 000 на gpt-4.1 и 500 000 на
// моделях нового поколения (её скрин лимитов, 14.09.2026). Именно из-за 30 000
// падал «Голос клиента»: полный материал не помещался в минуту.
const MODELS = [
  ['gpt-6-astra',   'GPT-6 Astra — самая сильная, лимит 500 000 в минуту'],
  ['gpt-5.6-sol',   'GPT-5.6 Sol — лимит 500 000 в минуту'],
  ['gpt-5.6-terra', 'GPT-5.6 Terra — лимит 500 000 в минуту'],
  ['gpt-5.6-luna',  'GPT-5.6 Luna — лимит 500 000 в минуту, дневной запас втрое больше'],
...
const T = {
  en: {
    ...
    lockTitle: 'Access',
    lockSub: 'Sign in with your account email and password',
    lockPlaceholder: 'Password',
    lockButton: 'Sign in',
    lockError: 'Wrong email or password. Try again.',
    signOut: 'Sign out',
    ...
    seedConfirmTitle: 'Confirm search phrases before checking Wordstat/Google Ads',
    seedConfirmSub: 'Edit, remove, or add phrases — these will be checked against real search volume. Automatic guessing can miss homonyms (e.g. "pop-up" also means fishing bait) or pick a term too narrow to have data.',
    seedAdd: '+ Add phrase',
    seedContinue: 'Continue →',
  ...
  ru: {
    ...
    lockTitle: 'Доступ',
    lockSub: 'Войдите под email и паролем своего аккаунта',
    lockPlaceholder: 'Пароль',
    lockButton: 'Войти',
    lockError: 'Неверный email или пароль. Попробуйте ещё раз.',
    signOut: 'Выйти',
    ...
    seedConfirmTitle: 'Подтвердите поисковые фразы перед проверкой в Wordstat/Google Ads',
    seedConfirmSub: 'Отредактируйте, удалите или добавьте фразы — по ним будет проверена реальная частотность. Автоматический подбор может пропустить омоним (например «попап» — это ещё и рыболовная насадка) или выбрать слишком узкую фразу, по которой нет данных.',
    seedAdd: '+ Добавить фразу',
    seedContinue: 'Продолжить →',
...
// оболочка. Разбираем в локальные имена, чтобы все места вызова ниже остались
// как были: файл вынесен, поведение не менялось.
const { signIn, refreshTokens, authFetch, getRefreshToken, clearTokens } = window.CAAuth;

// ── DB SYNC (Supabase, через api/projects.js) ────────────────────────────────
// localStorage остаётся источником истины для мгновенного чтения/записи (без сети,
// без риска потерять шаг дорогого прогона): каждый sv()/upsert() шлёт ПОЛНЫЙ объект
...
const hydrateFromDb = async () => {
  try {
    const r = await authFetch('/api/projects', { headers: dbHeaders() });
    if (!r.ok) return null;
...
// M10 — трекер публикаций. Данные живут ТОЛЬКО в Supabase (в отличие от M1–M7
// без localStorage-подстраховки): если БД не настроена, экран трекера прямо
// покажет «БД не настроена», а не тихо потеряет введённую публикацию/метрику.
const loadPublications = async (projectId) => {
  try {
    const r = await authFetch('/api/publications?project_id='+encodeURIComponent(projectId), { headers: dbHeaders() });
    if (!r.ok) return null;
    const d = await r.json();
    return Array.isArray(d.publications) ? d.publications : null;
  } catch { return null; }
};
const addPublication = async (pub) => {
  try {
    const r = await authFetch('/api/publications', { method:'POST', headers: dbHeaders(), body: JSON.stringify({kind:'publication', ...pub}) });
    if (!r.ok) return null;
    const d = await r.json();
    return d.publication || null;
  } catch { return null; }
};
const addMetricsSnapshot = async (snap) => {
  try {
    const r = await authFetch('/api/publications', { method:'POST', headers: dbHeaders(), body: JSON.stringify({kind:'metrics', ...snap}) });
    return r.ok;
  } catch { return false; }
};

// M8 — тренд-монитор: история снапшотов по (проекту, нише), только в Supabase (как M10).
const loadTrendSnapshots = async (projectId, niche) => {
  try {
    const r = await authFetch('/api/trends?project_id='+encodeURIComponent(projectId)+'&niche='+encodeURIComponent(niche||''), { headers: dbHeaders() });
    if (!r.ok) return null;
    const d = await r.json();
    return Array.isArray(d.snapshots) ? d.snapshots : null;
  } catch { return null; }
};
const addTrendSnapshot = async (snap) => {
  try {
    const r = await authFetch('/api/trends', { method:'POST', headers: dbHeaders(), body: JSON.stringify(snap) });
    if (!r.ok) return null;
    const d = await r.json();
...
// Счётчик платных вызовов Tavily за текущий модуль (для расчёта тарифов,
// 26.08.2026) — тот же боковой приём, что lastGptUsage: run() обнуляет перед
...
// Старые модели держат 30 000 токенов в минуту (≈75 000 знаков вместе с
// ответом) — под них и режем. У нового поколения лимит 500 000, там резать
// нечего: полный материал проходит целиком.
...
async function callGPT(system, user, temperature, maxTokens, попытка) {
  ...
  const res = await authFetch('/api/proxy', {
    ...
    body: JSON.stringify({model: мод, stream:false,
      ...
      ...(старая ? { max_tokens: maxTokens || 8000 }
                 : { max_completion_tokens: Math.max(maxTokens || 8000, 64000) }),
  ...
  // 27-29.08 владелица дважды получала голое «Error: API 429» и мы гадали.
  if (!res.ok) {
    let detail = '';
    try {
...
async function callSearch(query, opts) {
  searchCallCount++; // считаем попытку вызова Tavily — платится независимо от числа результатов
  try {
...
// Реальная частотность (M7): Yandex Wordstat (только для рынка России —
// маршрутизация в keywordSourceForMarket). null при недоступности/не
...
async function callWordstat(phrase, opts) {
  ...
  try {
    const res = await authFetch('/api/wordstat', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({phrase, ...(opts||{})}),
    });
...
// Реальная частотность (M7) для рынков вне России — Google Ads Keyword Planner
// (REQ-025). Та же форма ответа, что у callWordstat — fetchKeywordFrequencyData
...
async function callGoogleAds(phrase, market, lang, siteUrl) {
  ...
  try {
    const res = await authFetch('/api/google-ads', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({phrase, market, lang, ...(siteUrl?{siteUrl}:{})}),
    });
    if (!res.ok) return null;
    return await res.json();
  } catch { return null; }
...
// Карта сайта: какие страницы уже опубликованы. null при любой неудаче —
// вызывающий код считает это «карты нет» и работает как раньше.
async function callSitemap(url) {
  try {
    const res = await authFetch('/api/sitemap', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({url}),
    });
    if (!res.ok) return null;
    const d = await res.json();
    return Array.isArray(d.urls) && d.urls.length ? d : null;
  } catch { return null; }
}
...
// M3 VoC — реестр площадок по нише (ТЗ-M3-VOC.md, п.2). Тот же паттерн, что
// остальные вызовы к api/*.js. GET — существующие домены ниши; POST — массовый upsert.
async function callDomainRegistry(method, params) {
  try {
    if (method === 'GET') {
      const res = await authFetch('/api/domain-registry?niche=' + encodeURIComponent(params.niche));
      if (!res.ok) return null;
      const d = await res.json();
      return d.domains || null;
    }
    const res = await authFetch('/api/domain-registry', {
      method: 'POST', headers: {'Content-Type':'application/json'},
...
// по-русски: ни турецких форумов, ни отзывов на турецком не находилось в
// принципе. То есть выход на нерусские рынки был сломан на уровне сбора данных,
// хотя M7 под них уже был готов (Google Keyword Planner подключён 25.08).
//
// Почему через модель, а не таблицей переводов: языков в списке десять, и дело
// не в переводе слов. «Отзывы и жалобы» в разных странах живут на разных
// площадках, и запрос должен это учитывать — в Турции это Ekşi Sözlük, в другой
// стране своё. Модель это знает, а список площадок, захардкоженный мной, устареет
// и будет врать (уже наступали на это с реестром доменов 25.08).
//
// Русский путь НЕ ТРОГАЕТСЯ вообще: возвращаются те же строки, что были. Это
// единственный рынок, на котором сейчас реально работают, и рисковать им ради
// ещё не начатых нельзя.
const QUERY_PARTS_RU = {
  demand:           'спрос',
  demandForum:      'спрос отзывы форум',
  priceCompetitors: 'цена конкуренты обзор рынка',
  forumProblems:    'форум обсуждение проблемы с заявками и клиентами',
  marketTrends:     'тренды рынка объём',
  whichIndustries:  'для каких сфер бизнеса и отраслей применяется',
  losingLeads:      'какие отрасли и ниши теряют заявки и лиды на сайте',
  caseExamples:     'примеры применения по отраслям кейсы',
  usedInNiches:     'в каких нишах и отраслях используют',
  reviewsPrices:    'отзывы цены тарифы',
  alternativeCheap: 'альтернатива',
  cheaperSmall:     'дешевле небольшая компания',
  officialPrices:   'официальный сайт цены тарифы',
  fewLeads:         'мало заявок с сайта конверсия проблемы',
  forumComplaints:  'форум "не работает" OR "бесит" OR "разочарован"',
  howToIncrease:    'как увеличить заявки с сайта',
  discussion:       'обсуждение',
  reviewsDrawbacks: 'отзывы недостатки жалобы',
  notRecommend:     '"не рекомендую" OR "разочарован" проблемы поддержка',
  clientReviews:    'отзывы клиентов форум',
  communities:      'сообщества телеграм-каналы где общаются профессионалы',
  bloggers:         'блогеры эксперты кого читают и смотрят',
  howToChoose:      'как выбрать',
  forNiche:         'для',
  seoSemantics:     'seo продвижение семантика запросы',
  keywordStats:     'частотность запросов вордстат статистика',
};
let queryPartsCache = {};   // ключ «язык|рынок» — один вызов модели на прогон, не на модуль
async function localizedQueryParts(lang, market) {
  const target = String(lang || 'Russian');
  if (/^rus|^рус/i.test(target)) return QUERY_PARTS_RU;
  const key = target + '|' + market;
  if (queryPartsCache[key]) return queryPartsCache[key];
  try {
    const sys = 'Ты переводишь заготовки поисковых запросов для маркетингового исследования. Отвечай ТОЛЬКО JSON-объектом с теми же ключами, без пояснений и markdown-обёртки.';
    const user = 'Рынок исследования: ' + (market || '—') + '. Язык поиска: ' + target + '.\n\n'
      + 'Ниже заготовки поисковых запросов на русском. Переведи КАЖДУЮ так, как реальный человек на этом рынке набрал бы её в поиске.\n\n'
      + 'Это НЕ дословный перевод. Два правила:\n'
      + '1. Используй те слова, которыми в этой стране реально ищут. Дословный перевод русского маркетингового оборота часто не гуглится вообще.\n'
      + '2. Где речь об отзывах, жалобах, форумах и сообществах — добавь в запрос название площадки, где в ЭТОЙ стране такие обсуждения реально живут (например для Турции это ekşi sözlük). Если не уверен в площадке для этой страны — не выдумывай, оставь запрос без названия.\n\n'
      + 'Верни JSON с теми же ключами:\n' + JSON.stringify(QUERY_PARTS_RU, null, 1);
    const raw = await callGPT(sys, user, 0.2, 900);
    const m = raw.match(/\{[\s\S]*\}/);
    const obj = m ? JSON.parse(m[0]) : null;
    if (obj && typeof obj === 'object') {
      // Недостающие ключи добираем русскими — лучше один запрос не на том языке,
      // чем упавший сбор доказательств.
      const merged = { ...QUERY_PARTS_RU, ...obj };
      queryPartsCache[key] = merged;
      return merged;
    }
  } catch {}
  return QUERY_PARTS_RU;   // не смогли перевести — работаем как раньше, не падаем
}
...
async function gatherNicheEvidence(brief, lang) {
  ...
  const Q = await localizedQueryParts(brief.researchLang, market);
  ...
  for (const s of topics) {
    ...
    queries.push(s+' '+market+' '+(manual.length ? product+' '+Q.demand : Q.demandForum));
    // Конкуренция и цены
    queries.push(s+' '+market+' '+Q.priceCompetitors);
    // Живые обсуждения — подтверждают, что боль в этой вертикали реальная, а не предполагаемая
    queries.push(s+' '+market+' '+Q.forumProblems);
  }
  queries.push(product+' '+market+' '+Q.marketTrends);
  ...
  queries.push(product+' '+market+' '+Q.whichIndustries);
  queries.push(product+' '+market+' '+Q.losingLeads);
  queries.push(product+' '+market+' '+Q.caseExamples);
  queries.push((brief.niche||product)+' '+market+' '+Q.usedInNiches);
...
// ── ЛИНТЕР ИНОЯЗЫЧНЫХ ВСТАВОК (финальная проверка перед выгрузкой) ──────────
// Правило 8 системного промпта уже запрещает смешивать алфавиты, но модель нарушает
// его молча — на прогоне 20 июля в русском отчёте остались «Guardrails», «TOFU/BOFU»,
// «Case-study», «Try free for 7 days». Здесь проверяется ФАКТ выдачи, а не намерение.
// Ничего не исправляем автоматически: слепая замена сломала бы легитимную латиницу —
// названия конкурентов, дословные цитаты их CTA (M2 просит формулировку как есть), URL.
// Человек смотрит список и решает сам.
const LATIN_SCRIPT_LANGS = ['english','turkish','german','spanish','french','italian','portuguese','polish','dutch'];

// Аббревиатуры и метки статусов, которые промпты САМИ требуют оставлять латиницей
// (правило 5 системного промпта, вердикты M1.2, статусы гэп-анализа M2).
const LINT_ALLOWED = new Set(['tam','sam','som','jtbd','cjm','lpr','voc','cac','aov','ltv','arpu','roi','roas','seo','sem','smm','cta','ctr','cpc','cpm','cpa','cpl','kpi','usp','crm','erp','saas','paas','b2b','b2c','d2c','p2p','ui','ux','api','url','uri','id','pdf','csv','xls','html','css','json','ai','ml','go','maybe','no','win','parity','lose','confidence','high','medium','low','pro','lite','basic','premium','free','trial','vs','ok','rss','sms','faq','qr','utm','ab','crm']);

// Английские термины, у которых есть обиходный русский эквивалент. Это «жёсткие»
// находки: их появление в русском тексте — почти всегда небрежность модели.
const LINT_DICT = {
  guardrails:'ограничения / рамки', guardrail:'ограничение',
  tofu:'верх воронки', mofu:'середина воронки', bofu:'низ воронки',
  awareness:'осведомлённость', insight:'вывод', insights:'выводы',
  headline:'заголовок', copywriting:'копирайтинг', copy:'текст',
  landing:'лендинг', funnel:'воронка', pipeline:'воронка / конвейер',
  touchpoint:'точка касания', touchpoints:'точки касания',
  onboarding:'онбординг', retention:'удержание', churn:'отток',
  engagement:'вовлечённость', reach:'охват', storytelling:'сторителлинг',
  framework:'схема / модель', benchmark:'ориентир', roadmap:'дорожная карта',
  milestone:'веха', deadline:'срок', feedback:'обратная связь',
  research:'исследование', timeline:'сроки', trigger:'триггер',
  lead:'лид', leads:'лиды', offer:'оффер', bundle:'пакет',
  upsell:'допродажа', crossell:'кросс-продажа', downsell:'удешевлённое предложение',
  discovery:'выявление потребности', objection:'возражение', objections:'возражения',
  scarcity:'дефицит', urgency:'срочность', bottleneck:'узкое место',
  'case-study':'кейс', casestudy:'кейс', 'follow-up':'повторное касание',
  followup:'повторное касание', 'pain':'боль', 'pains':'боли',
  'value':'ценность', 'proposition':'предложение', 'journey':'путь клиента',
  'persona':'персона', 'segment':'сегмент', 'budget':'бюджет',
};

// Домены и бренды документа — латиница в них законна, из находок исключаем.
function collectBrandTokens(text) {
  ...
  while ((m = urlRe.exec(text))) {
    ...
    for (const part of host.split('.')) {
      if (part.length > 2 && !/^(ru|com|net|org|io|kz|by|ua|tr|de|uk|me|app|dev|info|biz)$/i.test(part)) brands.add(part.toLowerCase());
    }
  }
  const bareRe = /\b([a-z0-9][a-z0-9-]{1,})\.(?:ru|com|net|org|io|kz|by|ua|tr|de|uk|me|app|dev|cloud|online|store|site|info|biz)\b/gi;
  while ((m = bareRe.exec(text))) brands.add(m[1].toLowerCase());
  return brands;
}

// Возвращает { hard:[…], soft:[…] } — термины с русским эквивалентом и прочая латиница.
// Каждая находка: { term, count, sample } — sample нужен человеку, чтобы увидеть контекст.
function lintForeignInsertions(text, lang) {
  const target = String(lang || '').toLowerCase();
  if (!text || LATIN_SCRIPT_LANGS.some(l => target.includes(l))) return { hard: [], soft: [], skipped: true };

  const brands = collectBrandTokens(text);
  // Вычищаем то, где латиница законна по определению: ссылки, код, markdown-таблицу источников.
  const cleaned = String(text)
    ...
    .replace(/https?:\/\/\S+/gi, ' ')
    .replace(/\[[^\]]*\]\([^)]*\)/g, ' ')
    .replace(/\b[a-z0-9-]+\.(?:ru|com|net|org|io|kz|by|ua|tr|de|uk|me|app|dev|cloud|online|store|site|info|biz)\b/gi, ' ');
  ...
  while ((m = wordRe.exec(cleaned))) {
    ...
    if (LINT_ALLOWED.has(key)) continue;
    if (brands.has(key)) continue;
    // Аббревиатура заглавными (JTBD, CJM, TAM) — разрешена правилом 5, даже если не в списке.
    // Но словарь сильнее: TOFU/BOFU тоже заглавные, и это именно те вставки, из-за
    // которых линтер и понадобился — их глушить нельзя.
    if (!LINT_DICT[key] && raw.length <= 6 && raw === raw.toUpperCase()) continue;
    const bucket = LINT_DICT[key] ? hard : soft;
    const prev = bucket.get(key);
    if (prev) { prev.count++; continue; }
    const at = m.index;
    const sample = cleaned.slice(Math.max(0, at - 45), at + raw.length + 45).replace(/\s+/g, ' ').trim();
    bucket.set(key, { term: raw, count: 1, sample, suggest: LINT_DICT[key] || '' });
  }
  const byCount = (a, b) => b.count - a.count;
  return { hard: [...hard.values()].sort(byCount), soft: [...soft.values()].sort(byCount), skipped: false };
}
...
// ── M3 VoC ГИГИЕНА (ТЗ-M3-VOC.md, п.1-4) ────────────────────────────────────
// Извлекает таблицу по заголовку (та же логика распознавания, что parseMdTables),
// но возвращает диапазон СТРОК ДАННЫХ, а не готовые объекты — нужно для точечной
// перезаписи (отфильтровали цитаты → вставили обратно ровно туда же), не трогая
// остальной markdown (шапку/разделитель таблицы, соседние блоки).
function extractMdTableByHeading(fullText, headingRe) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (t.startsWith('|') && i + 1 < lines.length && isMdSeparator(lines[i + 1].trim())) {
      ...
      if (headingRe.test(heading)) return { headers, rows, dataStart: i + 2, dataEnd: j };
      i = j - 1;
    }
  }
  return null;
}
// Пересобирает ТОЛЬКО строки данных (шапка/разделитель не трогаются) — используется
// после фильтрации/дедупа, чтобы вернуть отфильтрованную таблицу обратно в full.
function spliceMdTableRows(fullText, tableInfo, newRows) {
  const lines = String(fullText || '').split('\n');
...
function extractSection(fullText, headingRe) {
  ...
  let start = -1;
  for (let i = 0; i < lines.length; i++) {
    if (headingRe.test(lines[i].trim())) { start = i; break; }
  }
  if (start < 0) return '';
...
// Отбрасывает строки, чья нормализованная-в-нижний-регистр цитата уже встречалась
// раньше в массиве — «одна цитата → одна персона» (ТЗ, Дефект 4, хеш-половина).
function hashDedupe(rows, quoteField) {
  const seen = new Set();
  return rows.filter(row => {
    const key = normalizeQuote(row[quoteField]).toLowerCase();
    if (!key || seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}
// Итоговый блок покрытия VoC по сегментам — приклеивается КОДОМ после ответа модели
// (тот же приём, что wordstatFullListBlock для M7): модель его не пишет и не видит,
// поэтому гейт нельзя обойти формулировкой в промпте.
function vocCoverageBlock(block07Rows, segmentField, threshold, note) {
  const bySegment = new Map();
  for (const row of block07Rows) {
    const seg = (row[segmentField] || '—').trim() || '—';
    bySegment.set(seg, (bySegment.get(seg) || 0) + 1);
  }
  const rows = [...bySegment.entries()].map(([seg, n]) =>
    '| ' + seg + ' | ' + n + ' | ' + (n >= threshold ? 'достаточно' : 'insufficient (' + n + '/' + threshold + ')') + ' |'
  ).join('\n');
  const noteLine = note ? '\n\n_' + note + '_' : '';
  return '\n\n#### VoC Coverage (проверено кодом)\n'
    + '| Сегмент | Верифицированных цитат | Статус |\n|---|---|---|\n'
    + (rows || '| — | 0 | insufficient (0/' + threshold + ') |') + noteLine + '\n';
}
...
// Домены конкурентов из BLOCK 06 M2 (колонка «Сайт/URL») — бесплатный источник для
// реестра площадок (ТЗ-M3-VOC.md, п.2): данные уже есть, отдельный поиск не нужен.
function competitorDomainsFromM2(m2Result) {
  if (!m2Result || !m2Result.content) return [];
  const tables = parseMdTables(m2Result.content);
  const t06 = tables.find(t => /BLOCK\s*06(?!_1)\b/i.test(t.heading || ''));
  ...
  // результат вида "xn--%20-8cdtc9cga7b1b9d" (пробел внутри). Валидный домен
  ...
  for (const row of rows06) {
    ...
    try {
      ...
      if (validHost.test(host)) domains.add(host);
    } catch {}
  }
  return [...domains];
}

// Разведка доменов по нише (ТЗ-M3-VOC.md, п.2) — ПОЛНОСТЬЮ автоматическая: несколько
// лёгких поисковых запросов находят, ГДЕ вообще обсуждают эту нишу (форумы/Q&A/
// отзовики), нужны только домены найденных страниц, не содержимое. Владелица прямо
// просила без ручного ввода — весь реестр наполняется поиском, а не списком от неё.
async function discoverNicheDomains(brief) {
  const market = brief.geoMarket || brief.geoCompany || '';
  const product = brief.niche || brief.name || '';
  const niches = (brief.selectedNiche||'').split(',').map(s=>s.trim()).filter(Boolean);
  const topic = niches.length ? niches[0] : product;
  const queries = [
    topic+' '+market+' форум обсуждение',
    topic+' '+market+' отзывы площадка',
    topic+' '+market+' вопрос ответ',
  ];
  const evidence = await gatherEvidence(queries, 3, 5, { maxItems: 20 });
  const domains = new Set();
  for (const e of evidence) {
    try { domains.add(new URL(e.url).hostname.replace(/^www\./, '')); } catch {}
  }
  return [...domains];
}

// Реестр площадок для ниши — GET существующего (переиспользуем, ТЗ: «строится один
// раз») ИЛИ собираем автоматически (конкуренты M2 + разведка поиском) и сохраняем.
async function ensureDomainRegistry(niche, m2Result, brief) {
  if (!niche) return [];
  const existing = await callDomainRegistry('GET', { niche });
  if (existing && existing.length) return existing.map(r => r.domain);

  const fromCompetitors = competitorDomainsFromM2(m2Result).map(d => ({ domain: d, category: 'competitor_site', source: 'auto' }));
  const discovered = (await discoverNicheDomains(brief)).map(d => ({ domain: d, category: 'discovered', source: 'auto' }));
  const seen = new Set();
  const union = [...fromCompetitors, ...discovered].filter(d => !seen.has(d.domain) && seen.add(d.domain));
  if (!union.length) return []; // новая ниша без конкурентов и без находок — поиск идёт открытым, как раньше

  await callDomainRegistry('POST', { niche, domains: union });
  return union.map(d => d.domain);
}

async function gatherVoCEvidence(brief, discoveredCompetitors, m2Result) {
  const market = brief.geoMarket || brief.geoCompany || '';
  const Q = await localizedQueryParts(brief.researchLang, market);
  const product = brief.niche || brief.name || '';
  ...
  for (const n of topics) {
    queries.push(n+' '+market+' '+Q.fewLeads);
    queries.push(n+' '+market+' '+Q.forumComplaints);
    queries.push(Q.howToIncrease+' '+n+' '+market+' '+Q.discussion);
  }
  // 2. Отзывы о конкурентах = сразу и боль, и язык, и подсказки по фичам
  for (const c of competitors) {
    queries.push(c+' '+Q.reviewsDrawbacks);
    queries.push(c+' '+Q.notRecommend);
  }
  ...
  const audienceTopic = topics[0] || product;
  queries.push(audienceTopic+' '+market+' '+Q.communities);
  queries.push(audienceTopic+' '+market+' '+Q.bloggers);

  // 5. ТУДА, ГДЕ ЛЮДИ ПИШУТ САМИ. Модель 15.09 объяснила пустой блок прямым
  ...
  // СМЕЖНАЯ БОЛЬ. Про сам продукт («ИИ-попап») в рунете почти не говорят —
  // ниша узкая, и живых обсуждений нет. Но ТА ЖЕ аудитория много говорит про
  // боль, ради которой продукт покупают: заявки теряются, трафик есть, а
  // заказов нет, корзину бросают. Это её собственный язык, и для банка живого
  // языка он годится — с пометкой, что цитата про смежную тему (решение
  // владелицы 15.09).
  const больЗаказчика = [brief.result, brief.pain, brief.problem, brief.niche]
    .map(x => String(x || '').trim()).filter(Boolean)[0] || product;
  const тема = topics[0] || product;
  const смежные = [
    больЗаказчика + ' форум обсуждение проблема',
    тема + ' не работает что делать форум',
    тема + ' почему не получается отзывы',
    'как решают ' + больЗаказчика + ' на практике обсуждение',
  ];
  for (const q of смежные) queries.push(q + ' ' + market);

  // Прямая речь узнаётся по кавычкам: так ищут цитату, а не статью о теме.
  for (const n of topics.slice(0, 2)) {
    queries.push('"' + n + '" ' + market + ' «мы пробовали» отзыв владельца');
    queries.push('"' + n + '" ' + market + ' «столкнулись с проблемой» обсуждение');
  }

  // Реестр площадок (ТЗ-M3-VOC.md, п.2) — ПОКА НЕ ограничивает сам поиск VoC.
  ...
  // себя) + случайных находок общих запросов «форум обсуждение» (не факт, что
  // это форумы вообще — среди находок был, например, обычный интернет-магазин
  // парфюмерии). Резкое сужение опаснее мусора от открытого поиска. Реестр
...
async function processM5Voc(full, niche) {
  ...
  if (!t07 && !t07a) return full; // нечего верифицировать — не трогаем full

  const findHeader = (headers, needle) => headers.find(h => h.toLowerCase().includes(needle.toLowerCase())) || null;
  const qKey07 = t07 && findHeader(t07.headers, 'цитата');
  const uKey07 = t07 && findHeader(t07.headers, 'url');
  const segKey07 = t07 && findHeader(t07.headers, 'сегмент');
  const qKey07a = t07a && findHeader(t07a.headers, 'цитата');
  const uKey07a = t07a && findHeader(t07a.headers, 'url');
  const qKey07b = t07b && findHeader(t07b.headers, 'цитата');
  const uKey07b = t07b && findHeader(t07b.headers, 'url');

  const items = [];
  if (t07 && qKey07 && uKey07) for (const r of t07.rows) if (r[qKey07] && r[uKey07]) items.push({ url: r[uKey07], quote: r[qKey07] });
  if (t07a && qKey07a && uKey07a) for (const r of t07a.rows) if (r[qKey07a] && r[uKey07a]) items.push({ url: r[uKey07a], quote: r[qKey07a] });

  const verifyResults = items.length ? await callVerifyQuotes(items) : [];
  // null = эндпоинт целиком не ответил (инфраструктурный сбой) — это НЕ то же самое,
  // что «страница не открылась» (per-item verified:false внутри results). В этом
  // случае НЕ вычищаем цитаты (модель могла быть права), а честно помечаем в блоке
  // покрытия, что верификация в этом прогоне не сработала.
  let verifiedSet = null, coverageNote = null;
  if (verifyResults) {
    verifiedSet = new Set(verifyResults.filter(x => x.verified).map(x => x.url + '|||' + normalizeQuote(x.quote).toLowerCase()));
  ...
  // dataStart/dataEnd у всех трёх таблиц посчитаны один раз от ИСХОДНОГО full — если
  // вставить/удалить строки в таблице, которая в тексте ВЫШЕ другой, все индексы ниже
  // неё съедут. Правка снизу вверх не мешает более ранним диапазонам.
  let result = full.replace(
    /(?:#{1,6}\s*|<<<)?CHART_DATA_START[\s\S]*?CHART_DATA_END(?:>>>)?/,
    '<<<CHART_DATA_START>>>\n' + JSON.stringify(chartData) + '\n<<<CHART_DATA_END>>>'
  );
  if (t07b) result = spliceMdTableRows(result, t07b, rows07b);
  if (t07a) result = spliceMdTableRows(result, t07a, rows07a);
  if (t07) result = spliceMdTableRows(result, t07, rows07);
...
async function gatherSeoEvidence(brief) {
  ...
  for (const n of topics) {
    ...
    queries.push(product+' '+Q.forNiche+' '+n);
    queries.push(Q.howToChoose+' '+product+' '+n+' '+market);
  }
  queries.push(product+' '+market);
  queries.push(product+' '+market+' '+Q.keywordStats);
  queries.push(product+' '+market+' '+Q.seoSemantics);
...
// Диапазон «последние 12 полных календарных месяцев» для сезонности —
// PERIOD_MONTHLY у Wordstat требует, чтобы toDate был последним днём месяца
// (см. api/wordstat.js), поэтому считаем от последнего ЗАВЕРШЁННОГО месяца,
// не от сегодняшней даты (текущий месяц ещё не полный).
function last12MonthsRange() {
  const now = new Date();
  const toDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 0, 23, 59, 59));
  const fromDate = new Date(Date.UTC(toDate.getUTCFullYear()-1, toDate.getUTCMonth()+1, 1, 0, 0, 0));
  return { fromDate: fromDate.toISOString(), toDate: toDate.toISOString() };
}

// Маршрутизация источника частотности по рынку (REQ-025, 25.08.2026): Yandex
// Wordstat реально работает только для русскоязычного рынка (доминирует в РФ,
// ~70%) — для остальных исследуемых рынков (Турция, ОАЭ, англоязычные) нужен
// Google Ads. Казахстан/Беларусь тоже идут в Google, хотя там у Yandex
// заметное меньшинство (~28-35%) — решение владелицы для первой версии: один
// источник на рынок, без слияния данных из двух API с разной логикой
// матчинга (Wordstat — точная строка, Google Ads — смысловые варианты).
function keywordSourceForMarket(market) {
  const m = String(market || '').toLowerCase();
  if (/росси|russia|\bрф\b/i.test(m)) return 'yandex';
  return 'google'; // всё остальное, включая KZ/BY и пустой рынок
}

// РЕГРЕССИЯ 25.08.2026, найдена владелицей на живом прогоне: старая эвристика
// «обрезать до первого „для“» ломается, когда «для» — естественная часть
// самого niche, не хвост-уточнение. Пример из реального брифа: niche =
// «инструмент для захвата лидов/умные попапы с ИИ» → обрезка дала «инструмент»
// (обычное слово «инструмент», ничего общего с продуктом) — Wordstat честно
// вернул реальную частотность магазина инструментов (vseinstrumenti.ru),
// модель корректно отказалась приписать эти числа своим кластерам про попапы
// («не замерено» — это сработавшая защита, не баг самого M7), но толку ноль:
// считать было нечего. Регулярка не может надёжно отличить «попап ДЛЯ сайта»
// (для = хвост-уточнение, можно резать) от «инструмент ДЛЯ захвата лидов»
// (для = часть смысла) на свободном тексте — доверяем модели, не regex.
//
// Дешёвый отдельный вызов вместо парсинга: 2-3 короткие ФРАЗЫ, которые
// человек реально вбил бы в поиск, ища ИМЕННО ТИП продукта — не выгоду,
// не боль, не описание. При сбое (сеть/парсинг) — старая эвристика как
// запасной вариант, лучше несовершенный якорь, чем совсем без частотности.
// Ошибка последнего подбора фраз — боковой канал, как lastGptUsage. Пусто, если
// всё прошло успешно. run() читает сразу после вызова и показывает владелице.
let lastSeedError = null;
async function extractWordstatSeeds(brief) {
  lastSeedError = null;
  const product = brief.niche || brief.name || '';
  // Запасного варианта «отрезать всё после для» больше НЕТ. Он давал мусор и,
  // хуже того, молча: ниша «инструмент для захвата лидов/умные попапы с ИИ»
  // превращалась в «инструмент», и это показывалось владелице как нормальная
  // подсказка. 29.08 так и вышло — вызов модели упал с 429, а она увидела не
  // ошибку, а плохую подсказку, и решила, что модель стала хуже подбирать.
  // Честнее вернуть пусто и сказать, что подобрать не вышло: фразы всё равно
  // утверждает человек, пустое поле он заполнит сам, а мусорную подсказку
  // может принять за рекомендацию.
  if (!product) return [];
  try {
    const sys = 'Ты подбираешь короткие поисковые фразы для проверки реальной частотности в Yandex Wordstat / Google Keyword Planner. Отвечай ТОЛЬКО JSON-массивом строк, без пояснений и markdown-обёртки.';
    const user = 'Продукт:\n'
      + 'Название: ' + (brief.name || '—') + '\n'
      + 'Ниша/описание: ' + product + '\n'
      + 'Формат: ' + (brief.format || '—') + '\n'
      + 'Услуги: ' + ((brief.services||[]).join(', ') || '—') + '\n\n'
      + 'Выбери 3-4 поисковые фразы — ровно то, что реальный человек вбил бы в поиск. НЕ выгоду, НЕ боль, НЕ описание пользы целиком (плохо: «инструмент для захвата лидов» — это описание, не запрос).\n\n'
      + 'Два правила:\n'
      + '1. Не только общая категория — минимум одна фраза ОТДЕЛЬНО под то, что именно ОТЛИЧАЕТ продукт (например если в описании есть ИИ/автоматизация — «умный попап» или «попап с ии» отдельным пунктом, не только общее «попап»). Без этого частотность найдётся для категории вообще, но не для того, чем продукт реально отличается.\n'
      + '2. Осторожно с однословными омонимами: если короткое слово в обычном поиске может значить что-то ДРУГОЕ, не связанное с продуктом (например «попап» — это ещё и рыболовная насадка для карпа, «капсула» — ещё и лекарство) — используй 2-словную фразу, которая снимает двусмысленность («попап для сайта», не просто «попап»), даже если она чуть длиннее. Проверь каждую однословную фразу на этот риск перед тем, как включить её как есть.\n\n'
      + 'Верни JSON-массив 3-4 строк: ["фраза1","фраза2","фраза3"]';
    const raw = await callGPT(sys, user, 0.2, 250);
    const m = raw.match(/\[[\s\S]*\]/);
    const arr = m ? JSON.parse(m[0]) : null;
    const seeds = Array.isArray(arr) ? arr.map(s=>String(s||'').trim()).filter(Boolean) : [];
    if (!seeds.length) lastSeedError = 'Модель вернула пустой список — впишите фразы вручную.';
    return seeds.slice(0, 4);
  } catch (e) {
    lastSeedError = 'Не удалось подобрать фразы: ' + (e.message || e) + '. Впишите их вручную.';
    return [];
  }
}

// Лексический фильтр Wordstat-ассоциаций: ПРОБОВАЛИ И ОТКАЗАЛИСЬ (25-29.08.2026).
// Четыре захода: «любое общее слово» → пропускал «захватывающие фильмы» под сид
// «захват лидов»; «ВСЕ корни сида» → снёс «AI попап» под сид «умный попап»;
// обратно мягкий → снова мусор. Финальный прогон 28.08 показал главное: фильтр
// выкидывал ХОРОШЕЕ (Wordstat отдаёт близкие по смыслу фразы вроде
// «всплывающее окно» к сиду «попап» — общего корня нет, фраза терялась) и
// оставлял МУСОР («обратка» делит корень с «форма обратного звонка») — 8 фраз
// вместо сотен. Отличить «то же самое другими словами» от «случайный общий
...
// заведомо мусорное), а не белый (оставить только похожее на сид). Белый
// список резал хорошее — «всплывающее окно» не содержит слова «попап» и
// терялось; чёрный такого сделать не может, он трогает только перечисленное.
// Список намеренно короткий и про заведомо чужие темы, а не про «похоже/не
// похоже»: тематическую релевантность судит модель, она понимает смысл.
// Не добавлять сюда «скачать», «бесплатно», «отзывы» и подобное — в разных
// нишах это законные коммерческие запросы («скачать попап для сайта»).
const NOISE_PATTERNS = [
  /\bторрент/i, /\bвзлом/i, /\bчит(ы|ов)?\b/i, /\bскачать мод/i, /\bмод(ы)? на\b/i,
  /\bвакансии?\b/i, /\bзарплат/i, /\bрезюме\b/i,
  /\bпорно/i, /\bсекс\b/i,
  /как пишется/i, /\bсинонимы?\b/i, /\bзначение слова/i,
];
...
// Реальная частотность для M7 — 2-3 «якорных» фразы дают уже несколько
// десятков похожих ИЗМЕРЕННЫХ фраз каждая, поэтому не нужно заранее знать все
// кластеры SEO-02 — источник сам их предлагает (см. keywordSourceForMarket).
// ВАЖНО: Wordstat матчит по точной строке, не по смыслу (в отличие от Tavily-
// поиска в gatherSeoEvidence, где длинная фраза — нормально). Живой тест
// подтвердил: «попап для сайта» → totalCount 157, 10 associations; «попап для
// интернет-магазина» → totalCount 2, пусто — хотя обе фразы из 3 слов. Длинная
// склейка product+niche+market почти всегда даёт 0 реальных запросов.
// Сезонность — ТОЛЬКО для Yandex в этой версии (Google Ads даёт помесячную
// историю через отдельный, ещё не подключённый endpoint) — для Google-рынков
// seasonality всегда [], M7 честно пишет «не замерено» по сезонности.
//
// 26.08.2026: сиды больше НЕ подбираются здесь автоматически — за три захода
// (regex → модель без правил → модель с правилами про омонимы) стало ясно,
// что ни одна автоматика не заменит человека, который мгновенно видит, что
// «попап» может значить «рыболовная насадка». extractWordstatSeeds теперь
// только ПРЕДЛАГАЕТ кандидатов — их утверждает/правит владелица в UI
// (showSeedConfirm/continueAfterSeeds в run()) ДО того, как тратится платный
// вызов Wordstat/Google Ads. Эта функция принимает уже готовый список.
async function fetchKeywordFrequencyData(brief, seeds, competitorDomains) {
  const market = brief.geoMarket || brief.geoCompany || '';
  const source = keywordSourceForMarket(market);
  // Потолок 12, а не 4: список отобран ЧЕЛОВЕКОМ вручную, тихо обрезать его до
  // первых четырёх — терять именно ту работу, ради которой делалась пауза
  // (владелица дополняла список, а фразы с 5-й молча не запрашивались).
  const uniqueSeeds = [...new Set(seeds)].filter(Boolean).slice(0, 12);
  const frequencies = [];
  const seen = new Set();
  const ideas = [];            // правая колонка Wordstat — подсказки, не ядро
  const seenIdea = new Set();
  for (const phrase of uniqueSeeds) {
    // numPhrases 200 (было 50, потолок API — 2000): владелице нужен рабочий
    // объём для SEO-плана — порядка 30 непересекающихся кластеров на нишу, а
    // отобрать 30 можно только из сотен. Цена та же: это ОДИН вызов на фразу
    // независимо от numPhrases, просто просим отдать больше строк за раз.
    // Узкая фраза столько вариантов и не даст — вернётся сколько есть, не ошибка.
    // 0-показов исключаем сразу здесь — источники их возвращают, но это шум, не спрос.
    const d = source === 'yandex'
      ? await callWordstat(phrase, { numPhrases: 200 })
      : await callGoogleAds(phrase, market, brief.lang);
    if (!d) continue;
    // Частотность САМОЙ подтверждённой фразы: Wordstat отдаёт её в totalCount, и
    // до 28.08 она молча выбрасывалась — брались только results/associations
    // (похожие фразы, которые предложил сам Wordstat). Из-за этого ключи, которые
    // владелица отобрала руками, единственные и оставались «не замерено», а в
    // таблицу попадали случайные ассоциации. Это и была причина жалобы «в таблице
    // не те ключи, что мы пишем». confirmed: true — их нельзя терять дальше.
    if (d.totalCount && !seen.has(phrase)) {
      seen.add(phrase);
      frequencies.push({ phrase, count: d.totalCount, confirmed: true });
    }
    // ТОЛЬКО results (левая колонка Wordstat — фразы, СОДЕРЖАЩИЕ запрос).
    // associations (правая колонка, «похожие запросы») в ядро больше НЕ идут.
    // Она построена на поведении («что ещё искали эти люди»), а не на словах
    // запроса, и по профессиональной практике это подсказка для новых идей, а
    // не готовая семантика. Прогон 28.08 показал цену смешивания: в списке
    // оказались «портал» (3 281 965), «скачать мод на трансформеров», «как
    // выглядит кляп», «ry7 ru интернет магазин» — ни одна из них не содержит
    // ни одного слова из подтверждённых фраз. Ассоциации сохраняем отдельно,
    // ниже они уходят в собственный раздел отчёта как идеи для расширения.
    for (const r of (d.results || [])) {
      if (!r.phrase || !r.count || seen.has(r.phrase)) continue;
      if (isNoiseKeyword(r.phrase)) continue;
      seen.add(r.phrase);
      // monthly/competition приходят только от Google (у Wordstat их нет) —
      // переносим как есть, ниже из них берётся сезонность без доп. вызова и
      // колонка «Конкуренция» с реальным числом вместо оценки модели.
      // Совпавшую с сидом фразу помечаем confirmed: у Google нет totalCount,
      // поэтому опорная фраза узнаётся только так.
      frequencies.push({
        phrase: r.phrase, count: r.count,
        ...(String(r.phrase).toLowerCase().trim() === String(phrase).toLowerCase().trim() ? { confirmed: true } : {}),
        ...(r.competition ? { competition: r.competition } : {}),
        ...(r.competitionIndex != null ? { competitionIndex: r.competitionIndex } : {}),
        ...(Array.isArray(r.monthly) && r.monthly.length ? { monthly: r.monthly } : {}),
      });
    }
    for (const r of (d.associations || [])) {
      if (!r.phrase || !r.count || seenIdea.has(r.phrase) || seen.has(r.phrase)) continue;
      if (isNoiseKeyword(r.phrase)) continue;
      seenIdea.add(r.phrase);
      ideas.push({ phrase: r.phrase, count: r.count });
    }
  }
  // Ключи по САЙТАМ КОНКУРЕНТОВ — только Google: GenerateKeywordIdeas принимает
  // URL как затравку («по каким запросам работает вот этот сайт»). У Wordstat
  // такой возможности нет вообще, а это ровно то, ради чего покупают key.so.
  // Домены берём из M2 (BLOCK 06), то есть бесплатно — они уже собраны.
  // Ограничение 3 сайта: каждый — отдельный вызов, а ценность быстро убывает.
  if (source === 'google' && Array.isArray(competitorDomains) && competitorDomains.length) {
    for (const domain of competitorDomains.slice(0, 3)) {
      const d = await callGoogleAds(null, market, brief.lang, 'https://' + domain);
      if (!d) continue;
      for (const r of (d.results || [])) {
        if (!r.phrase || !r.count || seen.has(r.phrase)) continue;
        if (isNoiseKeyword(r.phrase)) continue;
        seen.add(r.phrase);
        frequencies.push({
          phrase: r.phrase, count: r.count, fromCompetitor: domain,
          ...(r.competition ? { competition: r.competition } : {}),
          ...(r.competitionIndex != null ? { competitionIndex: r.competitionIndex } : {}),
          ...(Array.isArray(r.monthly) && r.monthly.length ? { monthly: r.monthly } : {}),
        });
  ...
  // Лексический фильтр УБРАН (29.08). Он задумывался как защита от плохих
  // АВТОМАТИЧЕСКИ подобранных сидов, но с 26.08 сиды подтверждает человек —
  // защита переехала выше по потоку. А на живых прогонах фильтр делал ровно
  // обратное задуманному: выкидывал хорошее и оставлял мусор. Wordstat отдаёт
  // фразы, близкие ПО СМЫСЛУ («всплывающее окно» к сиду «попап»), а фильтр
  // требовал общий корень — и такие фразы терял; при этом «обратка» и «туда
  // обратно как пишется» проходили, потому что делят корень с сидом «форма
  ...
  const seasonality = [];
  if (source === 'yandex') {
    const { fromDate, toDate } = last12MonthsRange();
    // Сезонность считаем в первую очередь для опорных фраз (их выбрал человек,
    // они точно попадут в таблицу), и только добиваем остаток самыми частотными
    // из найденных — иначе бюджет из 8 вызовов уходил бы на случайный топ,
    // который в таблицу может и не войти.
    const confirmedFirst = filtered.filter(f => f.confirmed);
    const rest = filtered.filter(f => !f.confirmed).sort((a,b)=>b.count-a.count);
    const topByCount = [...confirmedFirst, ...rest].slice(0, 8);
    for (const { phrase } of topByCount) {
      const d = await callWordstat(phrase, { mode:'dynamics', period:'PERIOD_MONTHLY', fromDate, toDate });
      const monthly = d && d.results ? d.results.filter(r=>r.count!=null) : [];
      if (monthly.length) seasonality.push({ phrase, monthly });
    }
  } else {
    // Google отдаёт помесячную историю (monthlySearchVolumes) ПРЯМО в ответе на
    // тот же запрос — отдельный вызов, как у Яндекса, не нужен вообще. Раньше мы
    // это поле не читали и честно писали «сезонность не замерено» для всех
    // не-российских рынков, хотя данные уже лежали в ответе.
    const withMonthly = filtered.filter(f => Array.isArray(f.monthly) && f.monthly.length);
    const confirmedFirst = withMonthly.filter(f => f.confirmed);
    const rest = withMonthly.filter(f => !f.confirmed).sort((a,b)=>b.count-a.count);
    for (const f of [...confirmedFirst, ...rest].slice(0, 8)) {
      seasonality.push({ phrase: f.phrase, monthly: f.monthly });
    }
  }

  ideas.sort((a,b)=>b.count-a.count);
  return { frequencies: filtered, ideas, seasonality, source: source === 'yandex' ? 'Yandex Wordstat' : 'Google Ads' };
}

// Полный список фраз+частотностей как ОТДЕЛЬНЫЙ блок markdown, собранный
// кодом, а не моделью — владелица делает это вручную в самой Wordstat
// (заходит и смотрит весь список похожих запросов) и хочет ровно то же самое
// автоматически: НИКАКОГО отбора/додумывания моделью, просто то, что вернула
// система, отсортировано по частотности, без нулей (уже отфильтровано выше).
function wordstatFullListBlock(frequencies, source, ideas) {
  let out = '';
  if (frequencies && frequencies.length) {
    const sorted = [...frequencies].sort((a,b)=>b.count-a.count);
    const rows = sorted.map(w => '| '+w.phrase+' | '+w.count+' |').join('\n');
    out += '\n\n#### Полный список запросов по '+(source||'Wordstat')+' (как есть, без отбора моделью)\n'
      + '| Фраза | Частотность (показов/мес) |\n|---|---|\n' + rows + '\n';
  }
  // Правая колонка Wordstat — отдельно и с оговоркой. Это НЕ семантическое ядро:
  // связь тут поведенческая («что ещё искали эти люди»), а не по словам запроса,
  // поэтому рядом с релевантным попадается совершенно постороннее. Ценность в
  // другом — здесь видны темы, до которых сами бы не додумались; годное отсюда
  // имеет смысл вручную добавить в список фраз на следующем прогоне.
  if (ideas && ideas.length) {
    const rows = ideas.map(w => '| '+w.phrase+' | '+w.count+' |').join('\n');
    out += '\n\n#### Похожие запросы (правая колонка ' + (source||'Wordstat') + ') — идеи для расширения, НЕ ядро\n'
      + 'Связь поведенческая, а не по словам запроса, поэтому среди релевантного попадается постороннее. '
      + 'Не использовать напрямую: пригодное можно вручную добавить в список фраз на следующем прогоне.\n\n'
      + '| Фраза | Частотность (показов/мес) |\n|---|---|\n' + rows + '\n';
  }
  return out;
}
...
// Заказчик (владелица, со слов её клиента, 24.08): «сравнение/конкуренты/аналоги»-
// запросы систематически находят только раскрученные бренды — это не баг промпта,
// а свойство самого поискового спроса: SEO-листиклы («Топ-10 лучших X») пишут именно
// про рыночных лидеров. Мелкую нишевую компанию не с кем сравнивать, если в evidence
// одни гиганты. Добавлены запросы, специально нацеленные НЕ на «лучших», а на то, где
// реально всплывают маленькие/нишевые игроки: форумные советы («посоветуйте»), прямые
// альтернативы уже известным конкурентам, «новый сервис»/стартап-формулировки.
async function gatherCompetitorEvidence(brief) {
  ...
  const queries = [
    ...
    // Нишевые/маленькие игроки — форумные советы и «альтернатива», не листиклы
    'посоветуйте '+(niche || product)+' '+market+' форум обсуждение',
    (niche || product)+' '+market+' новый сервис небольшая команда стартап',
    // Владелица 15.09: «нашлось всего пять, это ничего; он должен искать и
    // больших, и маленьких, разные сервисы». Пять запросов давали узкую
    // выдачу — добавляем ходы, которые вытаскивают РАЗНЫЕ слои рынка:
    // подборки и каталоги (широта), сравнения «против» (кто с кем меряется),
    // дешёвый и бесплатный конец (малые игроки), смежные формы продукта
    // (виджеты, плагины, конструкторы) и зарубежные ориентиры.
    product+' '+market+' топ сервисов подборка каталог',
    product+' '+market+' сравнение vs против какой выбрать',
    product+' '+market+' бесплатно дешёвый тариф альтернатива',
    product+' '+market+' виджет плагин конструктор для сайта',
    product+' alternatives best tools comparison',
  ];
  for (const c of named) {
    queries.push(c+' '+Q.reviewsPrices);
    queries.push(Q.alternativeCheap+' '+c+' '+market+' '+Q.cheaperSmall);
    // ТЗ-ИССЛЕДОВАНИЕ-ПОД-КОНТЕНТ.md п.5.6: цены/позиционирование — с сайта
    // САМОГО конкурента, не из статей-рейтингов. "-топ -рейтинг -обзор -сравнение"
...
async function gatherContentRadarEvidence(brief, competitors) {
  ...
  // Владелица 14.09: «каналов конкурентов всего шесть, это очень мало».
  // Каналов ровно столько, сколько их нашлось в выдержках, — значит расширять
...
// общий срез рынка.
async function gatherTrendEvidence(brief) {
  const market = brief.geoMarket || brief.geoCompany || '';
  const product = brief.niche || brief.name || '';
  const niche = brief.selectedNiche || '';
  const topic = niche || product;
  const queries = [
    topic+' '+market+' новости тренды',
    topic+' '+market+' новые игроки продукты запуск',
    topic+' '+market+' форум обсуждение свежие жалобы проблемы',
    topic+' '+market+' инфоповод событие',
  ];
  return gatherEvidence(queries, 8, 5, { days: 30 });
}
...
// ── SYSTEM PROMPT
// Глоссарий в ПРОМПТ, а не только в линтер (29.08.2026, решение владелицы).
// Линтер ловил английский жаргон в готовом отчёте и показывал его человеку —
// то есть работа по исправлению всё равно оставалась ручной. Владелица: нужно,
// чтобы термины СРАЗУ были на языке исследования, прямо в таблицах. Правило 8
// («не смешивай алфавиты») этого не давало: оно говорит ЧЕГО не делать, но не
// говорит, ЧЕМ заменить — модель не знает, как по-русски «guardrails» или
// «TOFU», и оставляет как есть. Отдаём тот же словарь LINT_DICT, по которому
// работает линтер, как обязательный список замен. Линтер остаётся проверкой
// того, что правило сработало: если он пуст — термины переведены.
// Для языков на латинице глоссарий не нужен — там английский жаргон не
// выглядит инородным, а словаря замен под них у нас и нет.
function glossaryRule(lang) {
  const target = String(lang || '').toLowerCase();
  if (LATIN_SCRIPT_LANGS.some(l => target.includes(l))) return '';
  const pairs = Object.entries(LINT_DICT).map(([en, ru]) => en + ' → ' + ru).join('; ');
  return '8a. ТЕРМИНОЛОГИЯ — переводи профессиональный жаргон, не оставляй его латиницей. '
    + 'Это относится и к таблицам, и к заголовкам колонок, и к тексту. Обязательные замены: '
    + pairs + '. '
    + 'Если нужного термина здесь нет — всё равно дай перевод на ' + lang
...
function buildSystem(brief, lang) {
  const lines = Object.entries(brief).filter(([,v])=>v).map(([k,v])=>{
    ...
    if(k==='regenNote') return ''; // не обычное поле брифа — отдельный CRITICAL-блок ниже (editorNote)
  ...
  return 'You are a senior market research analyst producing a professional-grade target audience research report.\n\n'+
    ...
    // «использовать подтверждённые источники», «расширить мониторинг
    // отзывов». Заказчику это не говорит ничего.
    '6. End each block with "Итог / Следующие шаги" (3-5 steps). ШАГИ — ПРО РЫНОК И ДЕЙСТВИЯ ЗАКАЗЧИКА, а не про наш процесс исследования. «Расширить мониторинг отзывов», «использовать подтверждённые источники», «собрать больше данных» — это указания самим себе, их писать НЕЛЬЗЯ. Пиши то, что заказчик может сделать со своим продуктом, ценой, текстами или каналами.\n'+
    '6б. В КАЖДОМ блоке после таблицы — одна строка «Вывод: …»: что из этой таблицы следует. Одно-два предложения, законченной фразой. Не «Комментарий:», не «Легенда:» — именно «Вывод:». Без него блок остаётся набором строк, из которого читатель сам должен догадаться, зачем он это читал.\n'+
    ...
    '7. Шкала уверенности — пользуйся ею единообразно во ВСЕХ таблицах модуля. САМУ ЛЕГЕНДУ НЕ ПЕЧАТАЙ: её показывает интерфейс под заголовком «Источники разведки». Значения: 5 — три и более независимых свежих источника; 4 — два источника; 3 — один источник; 2 — косвенные данные или эвристика; 1 — публичных данных нет.\n'+
...
Table (ONE ROW PER VERTICAL from 04_1 — list ALL, sorted by ИТОГО descending so max→min effectiveness is visible): Ниша | Спрос (1-5) | Конкуренция (1-5, где 5 = низкая) | Экономика (1-5) | Соответствие бизнесу (1-5) | ИТОГО (сумма) | Источники [n] | Уверенность (1-5) | Вердикт | Почему не идём
...
ПРАВИЛО ШИРИНЫ: не ограничивайся известными брендами. Список обязан включать игроков ВСЕХ слоёв, которые встретились в evidence, — и нишевых тоже.
СПИСОК ИЗ ПЯТИ СТРОК — ЭТО ОШИБКА, а не результат. Выпиши КАЖДОГО, кто назван в evidence хотя бы один раз: упомянутого вскользь в форумной ветке, названного в подборке, в сравнении «X против Y», в списке альтернатив. Малые и нишевые игроки нужны так же, как лидеры: по ним видно, чем рынок занят снизу.
Если игрок назван, но данных о нём мало — всё равно строка в таблице, недостающее помечай «нет данных (публично)». Пропускать его нельзя.
Если после этого строк всё равно меньше пятнадцати — напиши прямо, сколько нашлось и почему (узкая ниша, нет публичных источников), а не подавай короткий список как полный. Если evidence дал меньше 15 игроков, так и напиши: «в собранном материале найдено N игроков» — не добирай выдумкой.
...
МАСШТАБ — ОБЯЗАТЕЛЬНАЯ КОЛОНКА, НЕ ФОРМАЛЬНОСТЬ: заказчик прямо жаловался, что прошлая версия отчёта показывала только раскрученных рыночных лидеров — маленькую компанию не с кем было сравнивать. Оцени масштаб по evidence (упоминания охвата/клиентской базы/команды/года основания/масштаба присутствия) — «крупный» только если это видно из источника, иначе по умолчанию «нишевый-малый» или «неизвестно (мало данных)», НЕ «крупный» на основании узнаваемости названия. Список ДОЛЖЕН включать минимум несколько нишевых/малых игроков, если они есть в evidence выше — не ограничивайся только самыми известными брендами: сравнение с гигантами бесполезно компании нашего размера, если она не гигант сама. Если в evidence реально нет ни одного небольшого игрока (весь материал — про лидеров рынка) — честно напиши это одной строкой после таблицы («в собранном материале — только крупные игроки, нишевых конкурентов эта выдача не нашла»), не выдавай неполный список за полную картину.

ДУБЛИ ПРИ РЕБРЕНДИНГЕ — КРИТИЧНО: если evidence упоминает одну и ту же компанию (тот же URL/домен, или явно указано «ранее называлась X, теперь Y») под РАЗНЫМИ названиями — это ОДНА строка Comp_ID, не две. Используй самое свежее/актуальное название по дате evidence [n], старое название можно упомянуть в «Заметки» («ранее — Х»). Не создавай двух отдельных конкурентов из одной компании, сменившей название.
...
Format (таблица 1 — по сегментам, 4–6 строк):
РАЗБЕЙ НИШУ НА СЕГМЕНТЫ, а не подавай её одним куском. Две строки — это не разбор, а пересказ: владелице такой блок ничего не даёт. Бери сегменты из блока 04 этого исследования (описания аудитории) и деление по размеру/зрелости/способу покупки: например, малые и средние, крупные, те, кто уже пользуется похожим решением, и те, кто не пользуется ничем. Если сегментов действительно меньше четырёх — так и напиши, почему, а не добирай выдуманными.
...
Include 6-12 key competitors AND our company as one point with "isUs": true. Позиция конкурентов берётся из evidence; наша — ТОЛЬКО из брифа. Если в брифе нет цены или данных о качестве — не выдумывай выгодную позицию: ставь нас в середину (3) и прямо отметь это в тексте блока 06_1 фразой «позиция на графике — оценка по брифу, не замер». price: 1=cheapest, 5=most expensive. quality: perceived value/quality 1-5.`;
}

// M8 — Тренд-монитор. В отличие от M1-M7 это ПЕРИОДИЧЕСКИЙ снапшот (не единственный
...
function buildTrendPrompt(brief, lang, prevSnapshotText, m2Context, evidence) {
  ...
  const m2Block = m2Context ? `\nCOMPETITOR CONTEXT FROM M2 (for spotting changes among competitors):\n${m2Context.slice(0,1500)}\n` : '';
  return `${prevBlock}${m2Block}${evidenceSection(evidence)}
Fill module M8 «Trend Monitor» for this niche. ALL output in ${lang}. Answer: what changed in this niche RECENTLY (last ~30 days)?

BLOCK T1 — Тренды (Trends)
| Тема | Динамика (новая/растёт/затухает) | Что именно изменилось | Источники [n] | Рекомендация для контента |
Rules: только темы, реально подтверждённые evidence выше. Динамику "растёт"/"затухает" можно присвоить ТОЛЬКО если есть предыдущий снапшот для сравнения — иначе везде "новая" с пометкой "первый скан".

BLOCK T2 — Инфоповоды (Newsworthy)
| Инфоповод | Дата | Актуально до | Источник [n] | Как использовать в контенте |
Rules: «Актуально до» — честная оценка, когда инфоповод перестанет быть свежим (не выдумывай точные даты, если evidence не даёт — пиши «неделя-две» и т.п.).

BLOCK T3 — Дельта к прошлому снапшоту
Текстом (3-5 пунктов): что появилось нового с прошлого раза, что исчезло/затихло, что осталось стабильным. Если предыдущего снапшота нет — так и напиши.
...
Output JSON between <<<CHART_DATA_START>>> and <<<CHART_DATA_END>>>:
{
  "trends": [ { "topic": "тема", "status": "new/growing/fading", "recommendation": "..." } ],
  "newsworthy": [ { "item": "инфоповод", "validUntil": "срок актуальности", "howToUse": "..." } ],
  "delta": { "summary": "3-5 пунктов дельты одной строкой через ; " }
}
...
Format:
| Канал | Площадка | Чей (заказчик/конкурент/смежная ниша/медиа) | URL | Что видно публично | Дата |
...
Format (10–20 строк, если столько нашлось в выдержках):
| Канал | Площадка | Чей (заказчик/конкурент/смежная ниша/медиа) | Подписчики | Как часто выходит | Основной формат | Источник [n] |
Шесть строк — это не карта каналов, а обрывок: по ней нельзя понять, где вообще живёт разговор о нише. Выписывай КАЖДЫЙ канал, который встретился в выдержках, включая смежные ниши и отраслевые медиа, даже если про него видно только имя и площадку. Число подписчиков не нашлось — «не замерено», но строку оставляй.

═══════════════════════════════════════════════
BLOCK 24A — Что залетает (24A_Top_Content)
═══════════════════════════════════════════════
Task: 12–20 единиц контента с НАИБОЛЬШИМ откликом относительно канала. Владелица 14.09: «вариантов маловато» — бери из выдержек всё, что видно, а не только самые яркие. Именно
...
ПРОСМОТРЫ ВИДНЫ НЕ ВЕЗДЕ. Telegram их прячет, часть YouTube-страниц отдаёт без счётчика. Это НЕ повод выбросить строку: в колонке «Просмотры» пиши «не замерено», а отбирай единицы по тому, что видно, — числу реакций и комментариев, закреплению в канале, тому, что на пост ссылаются в других местах. Строка без просмотров, но с дословным хуком и разбором формата полезнее, чем отсутствие строки. Пустая таблица здесь — худший из возможных ответов: заказчик заплатил за разбор чужого контента и не получил ни одной единицы.

ХУК — ДОСЛОВНО. Не «цепляющий заголовок про конверсию», а та фраза, которая стоит первой строкой поста или первой секундой ролика. Если в выдержке фразы не видно — «не видно из источника». Пересказ хука своими словами бесполезен: его нельзя ни повторить, ни переделать.
...
Если в 24A получилось меньше трёх единиц — так и напиши одной строкой: «материала для паттернов не хватило, собрано N единиц» — и не выдумывай обобщения на пустом месте. Три и больше — таблица обязана быть заполнена: обобщать по трём единицам можно, помечая это в колонке «На чём основан».
...
Считай только по тем показателям, которые реально видны в собранном материале: длительность ролика, число реакций, частота публикаций, длина поста. Показатель, для которого чисел нет, в таблицу не вписывай вовсе — строка с пустой медианой хуже отсутствия строки. Если не набралось ни одного показателя хотя бы на пяти единицах — вместо таблицы одна строка: «бенчмарков не набралось: у площадок этой ниши публичных чисел слишком мало».

${clientSocials(brief).length ? `═══════════════════════════════════════════════
BLOCK 24D — Каналы заказчика (24D_Client_Channels)
═══════════════════════════════════════════════
Task: По КАЖДОМУ каналу самого заказчика (список в начале промпта) — одна
строка. Это точка отсчёта для плана: реальный ритм, а не желаемый. Числа —
только видимые на публичной странице; чего не видно — «не замерено».
Медиана просмотров/отклика — именно медиана, и рядом число публикаций, на
которых посчитана. «Топ публикаций» — до трёх лучших у этого канала:
тема — формат — URL, разделяй записи точкой с запятой.

Format:
| Площадка | Ссылка | Постов за период | Период | Последняя публикация | Форматы | Медиана просмотров | По скольким публикациям | Топ публикаций (тема — формат — URL; …) | Источник [n] |
...
СКОЛЬКО НАШЛОСЬ — СТОЛЬКО И ВЫПИСЫВАЙ. Числа в задании (20–30 и подобные) — это ориентир, а НЕ условие выдачи. Три честные цитаты лучше пустой таблицы с объяснением, почему её нет: по пустой таблице тексты всё равно напишут, только языком редактора. Пустую таблицу выдавай ТОЛЬКО если в материале нет вообще ни одного высказывания живого человека.

КТО СЧИТАЕТСЯ ЦЕЛЕВОЙ АУДИТОРИЕЙ — РОЛЬ, А НЕ ТЕМА РАЗГОВОРА. Годится любое высказывание человека той же роли (владелец бизнеса, маркетолог, тот, кто отвечает за заявки и продажи) — даже если он говорит не про наш инструмент, а про смежную боль: теряются заявки, трафик есть — заказов нет, реклама не окупается, клиенты уходят с сайта. Это и есть его язык, и он нужен нам именно такой. Такие цитаты помечай в теме словами «смежная боль».
НЕ годится речь ДРУГОЙ роли: покупатель интернет-магазина — не наша аудитория, он говорит о доставке и товаре, а не о заявках. Отсекай по РОЛИ говорящего, а не по тому, упомянут ли наш продукт.
...
ИНТЕНСИВНОСТЬ — не то же самое, что частотность, и для приоритизации она важнее. Оценивай по эмоциональности формулировки и по тому, сказано ли это без наводящего вопроса: «бесит, второй год мучаемся» — высокая; «хотелось бы поудобнее» — низкая, даже если встретилось десять раз. Боль, упомянутая трижды с яростью, важнее упомянутой десять раз вскользь: первая двигает людей покупать, вторая нет.

СВЕЖЕСТЬ: цитаты за последние 12 месяцев весомее старых — рынок и продукты меняются. Если цитата старше, это не повод её выбрасывать, но пометь возраст в «Комментарий», чтобы заказчик не строил на ней решение как на сегодняшней.

СМЕЩЕНИЕ ВЫБОРКИ — обязательная строка ПОД таблицей. Поиск, который принёс эти цитаты, шёл в основном по жалобам и отзывам, а на отзовиках и форумах пишут в основном недовольные и опытные пользователи. Значит картина смещена: молчаливое большинство и те, кому всё нравится, сюда не попали. Напиши это одной строкой прямым текстом. Без такой пометки заказчик прочтёт список болей как «мнение рынка», а это мнение самой громкой его части.

═══════════════════════════════════════════════
ЕСЛИ ПРО САМ ПРОДУКТ ЛЮДИ НЕ ГОВОРЯТ — БЕРИ СМЕЖНУЮ БОЛЬ. В узкой нише живых обсуждений самого инструмента может не быть вовсе, и пустой банк языка — худший исход: тогда тексты пишутся языком редакторов, а не покупателей. Бери дословные высказывания ТОЙ ЖЕ аудитории о боли, ради которой продукт покупают (теряются заявки, трафик есть — заказов нет, бросают корзину, реклама не окупается). В колонке темы пиши «смежная боль», чтобы читатель видел: это язык про проблему, а не про наш инструмент. Такие цитаты считаются наравне с прямыми — они и есть голос клиента.
...
Format:
| Кластер намерений | Примеры запросов | Уровень осведомлённости | Роль в воронке (реклама/лид-магнит/лендинг/FAQ) | Формула заголовка/оффера | Доказательство/артефакт | Мера спроса (Trends/подсказки/частота) | Примечание |
...
Типы, которые ОБЯЗАТЕЛЬНО рассмотреть — по каждому строка, если в evidence есть хоть намёк:
1. **Ничего не делать / оставить как есть** — самый частый и самый недооценённый конкурент. Человек живёт с проблемой, потому что она не настолько болит, некогда разбираться или он не верит, что это решаемо.
...
═══════════════════════════════════════════════
Task: Собери карту площадок, где эта аудитория реально присутствует. Отвечает на два вопроса заказчика: ГДЕ ПУБЛИКОВАТЬСЯ и С КЕМ СОТРУДНИЧАТЬ.

Format:
| Площадка/канал | Тип площадки | URL | Что аудитория там делает | Признак живости | Кого там читают | Как использовать | Источники [n] |
Пояснения к колонкам: «Тип площадки» — форум, соцсеть, мессенджер, отзовик, видео, отраслевое медиа, сообщество. «Что делает» — обсуждает, жалуется, ищет решение, учится. «Признак живости» — даты сообщений, число участников или просмотров, и ТОЛЬКО если это видно в выдержке. «Кого читают» — авторы, эксперты, блогеры, если они названы. «Как использовать» — публиковаться, участвовать в обсуждениях или искать партнёрство.

ЖЁСТКОЕ ПРАВИЛО ИСТОЧНИКОВ: каждая строка — ТОЛЬКО площадка, которая реально встретилась в evidence выше, со ссылкой [n]. Не перечисляй площадки «по общим соображениям» («ну, наверное, они сидят в Telegram и на VC») — такой список бесполезен, заказчик и сам это предположит. Если по какой-то колонке в выдержке данных нет — пиши «не видно из источника», а не догадку. Лучше 4 честные строки с [n], чем 15 придуманных.

ОТДЕЛЬНО ПОМЕТЬ смещение выборки: большая часть поиска шла по жалобам и отзывам, поэтому площадки жалоб в выдаче представлены сильнее, чем места, где аудитория просто живёт. Если это так — одной строкой под таблицей напиши, что карта смещена в сторону площадок с жалобами и какие типы площадок стоит проверить вручную.

═══════════════════════════════════════════════
BLOCK 07B — Hook Bank (07B_Hook_Bank)
═══════════════════════════════════════════════
Task: From the verbatim quotes you already collected in BLOCK 07/07A above, SELECT (do not invent) 5-10 that work well as the FIRST LINE of a short-form post or carousel cover — short, emotional, first-person, phrased the way the audience itself would recognise as "my own words". Light trimming of a long quote down to its punchy core is fine; writing new phrasing is not — every hook must trace back to an exact quote already present in BLOCK 07 or 07A of THIS output.

Format:
| # | Цитата-хук | Тема/боль | Площадка | URL |

If fewer than 5 quotes in BLOCK 07/07A actually qualify as strong hooks — output fewer, honestly. Do not pad with weak or off-target quotes just to reach 5.
...
{
  "hookBank": [
    { "quote": "exact quote text as written above", "theme": "тема/боль", "url": "..." }
  ]
}
...
function buildM6Prompt(brief, lang, prev, coverage) {
  ...
  // Гейт покрытия VoC (ТЗ-M3-VOC.md п.4) — считан кодом из M3, передан ОТДЕЛЬНО от
...
═══════════════════════════════════════════════
Task: From the fears/doubts in BLOCK 12 above (and any barrier patterns already visible in this research), synthesize 2-4 HARD RULES for content generation — things THIS audience will not forgive (bare promises without proof, invented statistics, guaranteed results, etc). Every rule must trace back to a specific fear/doubt already found above — not generic marketing advice invented from nothing.

Format (2-4 rows):
| # | Правило (что нельзя делать в контенте) | На основе какого страха/барьера (ссылка на строку BLOCK 12) |

Style example (not the content — your niche's rules will differ): «Никогда не пиши "гарантированно даст рост в разы/проценты" — только "может дать", "в теории"» — на основе страха «обещания без доказательств».
...
ПУТЬ РАЗБИВАЙ НА 6–8 ЭТАПОВ, НЕ КРУПНЕЕ. Четыре этапа («узнал — сравнил — купил — вернулся») заказчик и так знает: по такой лесенке нельзя понять, на каком шаге он теряет людей и что именно там показывать. Опорный набор шагов: (1) осознал проблему, но решения ещё не ищет; (2) ищет способ решить, формулирует запрос; (3) сравнивает варианты, включая «ничего не делать» и «сделать руками»; (4) проверяет именно нас — отзывы, кейсы, цена; (5) сомнение или внутреннее согласование перед оплатой; (6) первая попытка пользоваться; (7) первый результат; (8) повторная покупка или рекомендация. Шаг, которого на этом рынке нет, убирай — но склеивать несколько шагов в один «выбор» нельзя.

По КАЖДОМУ этапу — отдельная строка на каждую персону. Если для этапа нечего написать в колонке, пиши «не видно из источника», а не объединяй этапы.
...
Output JSON between <<<CHART_DATA_START>>> and <<<CHART_DATA_END>>>:
{
  ...
  "guardrails": [ "hard rule text, from BLOCK 12B above" ]
}`;
}
...
СТАТУС У ВСЕХ ГИПОТЕЗ — «ждёт проверки». Исследование ничего не проверяло: оно предлагает, что проверить. «Проверена», «подтверждена», «отклонена» ставить НЕЛЬЗЯ ни в одной строке — заказчик прочтёт это как результат теста, которого не было (владелица 15.09).

ССЫЛКИ НА ГИПОТЕЗЫ ДОЛЖНЫ ВЕСТИ НА СУЩЕСТВУЮЩИЕ. Если ниже в целях или в главной ставке ты ссылаешься на H01, H03 — эти строки обязаны быть В ЭТОЙ таблице с теми же номерами. Придуманный номер превращает цель в ссылку в никуда.
...
| Персона | Архетип ЦА | Архетип бренда | Ключевые мотивы | Страхи/табу | Эмоц. тон | Визуальные приёмы — что делать | Визуальные приёмы — чего избегать | Словесные приёмы — что делать | Словесные приёмы — чего избегать | Примеры сообщений |

═══════════════════════════════════════════════
BLOCK 20 — Content System (20_Content_System)
═══════════════════════════════════════════════
Format (20–30 rows):
| Segment_ID | Уровень осведомлённости | Формат контента | Цель (микро-KPI) | Крючок/1-й кадр | Визуальный мотив | Доказательства/цифры | CTA | Варианты A/B | Источники |

═══════════════════════════════════════════════
BLOCK 21 — Creative Briefs (21_Creative_Briefs)
═══════════════════════════════════════════════
Format (8–12 briefs):
| Brief_ID | Сегмент | Уровень осведомлённости | Цель теста | Сообщение | Визуал | Артефакты | Материалы/форматы | Ограничения | Источники [n] |
...
// siteMaps — что УЖЕ опубликовано: у клиента и у конкурентов. Идея из открытой
...
function buildM8Prompt(brief, lang, prev, evidence, wordstatData, siteMaps, geo) {
  ...
  // ничего не нашёл, wordstatData пуст и модуль ведёт себя как раньше («не замерено»).
  ...
  const wordstatBlock = freq.length
    ? '\n═══════════════════════════════════════════════\nREAL FREQUENCY DATA ('+srcLabel+', measured just now)\n═══════════════════════════════════════════════\n'
      + freq.map(w=>w.phrase+': '+w.count+'/мес'
          + (w.competition ? '   конкуренция: '+w.competition+(w.competitionIndex!=null?' ('+w.competitionIndex+'/100)':'') : '')
          + (w.confirmed?'   ← ОПОРНАЯ ФРАЗА (выбрана владелицей)':'')).join('\n')
      + '\n\nЭто РЕАЛЬНО измеренные числа (не из evidence, не выдумка) — используй их как приоритетный источник частотности для SEO-02.\n'
      + 'Фразы с пометкой «ОПОРНАЯ ФРАЗА» владелица отобрала вручную как главные для ниши. Каждая из них ОБЯЗАНА стать отдельным кластером в SEO-02 со своей измеренной частотностью — не заменяй их близкими по смыслу ассоциациями и не своди несколько опорных фраз в один кластер. Остальные фразы блока — материал для дополнительных кластеров.\n'
      + 'Где рядом с фразой указана «конкуренция» — это ИЗМЕРЕННАЯ величина от источника (LOW/MEDIUM/HIGH и индекс 0-100), а не оценка. Для таких кластеров бери её в колонку «Конкуренция» как есть, с пометкой источника, вместо собственной прикидки. Свою оценку давай только там, где измеренной величины нет.\n'
    : '';
  // Помесячная динамика за 12 месяцев — только для топ-фраз, только для Yandex
  // (см. fetchKeywordFrequencyData — Google Ads пока без сезонности).
  // Модель сама решает, растёт/падает/сезонит фраза, глядя на РЕАЛЬНЫЕ 12 чисел —
  // не придумывает «зимний пик», а описывает то, что реально в массиве.
  const seasonBlock = seas.length
    ? '\n═══════════════════════════════════════════════\nREAL SEASONALITY DATA ('+srcLabel+', monthly, last 12 months)\n═══════════════════════════════════════════════\n'
      + seas.map(w=>w.phrase+':\n'+w.monthly.map(m=>'  '+m.date.slice(0,7)+': '+m.count).join('\n')).join('\n')
      + '\n\nЭто РЕАЛЬНЫЕ помесячные числа — опиши на их основе сезонность словами (например «пик в декабре-январе, спад летом» или «стабильно весь год», если колебания небольшие). Не выдумывай сезонность для фраз, которых нет в этом блоке.\n'
    : '';
  const own = (siteMaps && siteMaps.own) || null;
  const rivals = (siteMaps && siteMaps.competitors) || [];
  const listUrls = us => us.slice(0, 120).join('\n');
  const sitemapBlock = (own || rivals.length)
    ? '\n═══════════════════════════════════════════════\nУЖЕ ОПУБЛИКОВАННЫЕ СТРАНИЦЫ (карты сайтов, прочитаны только что)\n═══════════════════════════════════════════════\n'
      + (own ? 'НАШ САЙТ (' + own.origin + '), страниц: ' + own.urls.length + (own.truncated ? '+ (список обрезан)' : '') + '\n' + listUrls(own.urls) + '\n\n' : '')
...
Допустимы ДВА источника числа частотности:
1. Блок REAL FREQUENCY DATA выше (если есть) — бери число оттуда напрямую, в источнике пиши «${srcLabel}» вместо [n].
2. Конкретная выдержка evidence — и тогда рядом ОБЯЗАТЕЛЬНА ссылка [n].
Если кластера нет ни там, ни там — пиши «не замерено». НЕ придумывай ни точное число, ни диапазон.
- Выдуманная частотность вида «2 000–3 500 /мес» или «900–1200» без ${srcLabel} и без [n] — ГРУБАЯ ОШИБКА и главный дефект этого модуля. Лучше честное «не замерено», чем правдоподобные выдуманные цифры: заказчик планирует по ним контент и бюджет.
- Ссылка на wordstat.yandex.ru/ads.google.com как «источник» без конкретного числа ИЗ блока выше или ИЗ evidence не считается подтверждением.

SEO-01 — Search Competitors (страницы, которые РЕАЛЬНО находятся по целевым запросам):
| URL | Найдено по запросу | Тип страницы | О чём страница | Заголовок страницы | Чем сильна | Источники [n] |
...
| Кластер | Тип (инфо/коммерч/навигац) | Примеры запросов | Частотность (число) | Группа (ВЧ/СЧ/НЧ) | Сезонность | Конкуренция (оценка + обоснование) | Приоритет | Тип страницы |
...
1. ОТБРОСЬ ОМОНИМЫ И ЧУЖИЕ ТЕМЫ. Инструмент замера ищет по форме слова, а не по смыслу, поэтому в выгрузку попадают фразы про совершенно другое, случайно делящие слово с продуктом. Примеры реального мусора из прошлых прогонов: «обратка» (202427/мес — это про автомобили/логистику, а не про форму обратного звонка), «туда обратно как пишется» (правописание), «попапы для рыбалки» (рыболовная насадка), «захватывающие фильмы» (кино, а не захват лидов). Высокая частотность НЕ делает такую фразу кластером — наоборот, она портит и таблицу, и график. Отбрасывай их молча, не объясняя в таблице.
2. СОРТИРУЙ ПО ЧАСТОТНОСТИ, начиная с самых ходовых. Верх таблицы — то, что реально ищут.
3. РАЗМЕТЬ ГРУППУ по частотности — это рабочая классификация SEO: ВЧ (высокочастотные — верхняя часть выгрузки по этой нише), СЧ (среднечастотные), НЧ (низкочастотные, длинный хвост). Границы определи САМ по фактическому распределению чисел в этой выгрузке, а не по универсальным порогам: в узкой B2B-нише 200 показов — это уже ВЧ, в массовой — НЧ. Нужны все три группы: по ВЧ пишут опорные статьи, по НЧ — точечные, и именно длинный хвост не даёт статьям конкурировать друг с другом.
4. ОБЪЁМ: ориентир — около 30 кластеров, если материала хватает (нижняя граница осмысленного — 12). Заказчику нужен рабочий объём для контент-плана: по одному кластеру на статью, чтобы статьи не перекрывались по ключам. Таблица из 5-8 строк при сотнях фраз в выгрузке — недоработка. Группируй близкие формулировки в один кластер (в «Примеры запросов» перечисляй варианты), но не сливай в кучу разные намерения — два разных намерения это два кластера и две статьи.

SEO-03 — Content Audit (по сайтам из evidence И по картам сайтов конкурентов, если они есть в блоке УЖЕ ОПУБЛИКОВАННЫЕ СТРАНИЦЫ — карта показывает ВЕСЬ их контент, а не те несколько страниц, что попали в поиск):
...
function buildReportPrompt(brief, results, lang) {
  // Задача 5 от контент-машины: «нужно, чтобы отчёты по нишам собирались в одну
  // кучу». Разбор показал, что дело не в сборке, а в двух поломках при нескольких
  // нишах:
  //  1. Секции шли БЕЗ указания ниши: при двух нишах модель видела два одинаково
  //     озаглавленных блока «=== M2: Competitors ===» и не могла их различить —
  //     конкуренты разных ниш сливались в одну секцию.
  //  2. completedIds шёл с дублями (results.map по всем нишам), поэтому в
  //     STRUCTURE список секций повторялся: «# Конкуренты (M2)» дважды, без
  //     пояснения, что это разные ниши.
  // Тот же класс ошибки, что был в buildFullMd (группировка по нишам).
  const ok = results.filter(r => !r.failed && !/^Error:/.test(String(r.content||'').trim()));
  const completedIds = [...new Set(ok.map(r => r.id))];
  const nicheList = [...new Set(ok.filter(r => isPerNiche(r.id)).map(r => r.niche || ''))].filter(Boolean);
  const multi = nicheList.length > 1;
  ...
  const body = r => {
    ...
    return '=== ' + r.id + (r.niche ? ' [НИША: ' + r.niche + ']' : '') + ': ' + (m ? m.title : r.id) + ' ===\n'
      + String(r.content || '').slice(0, 12000);
  };
  const globalSec = ok.filter(r => !isPerNiche(r.id)).map(body);
  // Сгруппировано по нишам, а не вперемешку: модель должна видеть, где кончается
  // одна ниша и начинается другая.
  const nicheSec = nicheList.map(n =>
    '########## ДАННЫЕ ПО НИШЕ: ' + n + ' ##########\n\n'
    + ok.filter(r => isPerNiche(r.id) && (r.niche || '') === n).map(body).join('\n\n---\n\n')
  );
  const sections = [...globalSec, ...nicheSec].join('\n\n---\n\n');
  ...
  return 'Write a professional market research report. ALL output in '+lang+' only. No fabricated data. No sections for modules not in: '+completedIds.join(', ')+'.'+companyBlock+'\n\n'+
    ...
    (multi
      ? '## Сравнение исследованных ниш\n[ОДНА таблица, строки — исследованные ниши ('+nicheList.join(', ')+'), '
        + 'колонки: Ниша | Кто покупатель | Главная боль | Сколько конкурентов и кто сильнейший | Спрос по ключам | '
        + 'Чем мы можем отличиться | Насколько уверены в данных (1-5). Заполняй ТОЛЬКО из данных этих ниш выше, не выдумывай. '
        + 'Под таблицей 2-3 строки: чем ниши отличаются по смыслу и какую стоит вести первой — с обоснованием из таблицы.]\n'
      : '')+
    '## Фокус прямо сейчас (3 конкретных действия из реальных данных)\n\n'+
    // При нескольких нишах модульные секции идут ВНУТРИ раздела своей ниши.
    // Раньше completedIds шёл с дублями, и структура просила «# Конкуренты (M2)»
    // дважды подряд — без указания, что это разные ниши.
    (multi
      ? nicheList.map(n =>
          '# Ниша: ' + n + '\n'
          + completedIds.filter(id => isPerNiche(id)).map(id => {
              const m = MODULES.find(x => x.id === id);
              return '## ' + (m ? (m.titleRu || m.title) + ' (' + id + ')' : id)
                + '\n[раздел по ЭТОЙ нише: данные, таблицы и блок «Что это значит для вас:». '
                + 'Бери только строки, помеченные НИША: ' + n + ' — не смешивай с другой нишей]';
            }).join('\n\n')
        ).join('\n\n')
        + (completedIds.some(id => !isPerNiche(id))
            ? '\n\n' + completedIds.filter(id => !isPerNiche(id)).map(id => {
                const m = MODULES.find(x => x.id === id);
                return '# ' + (m ? (m.titleRu || m.title) + ' (' + id + ')' : id) + '\n[общий раздел по проекту, не по нише]';
              }).join('\n\n')
            : '')
      : completedIds.map(id => {
          const m = MODULES.find(x=>x.id===id);
    ...
    '# Выводы\n'+
    (multi ? '## Итог по каждой нише (по 2-3 строки на нишу, что с ней делать)\n' : '')+
...
function ModuleCharts({ moduleId, chartData }) {
  ...
  try {
    ...
    if (moduleId === 'M8' && d.semanticClusters?.length > 0) {
      ...
      // почти пустым. Порядок по убыванию — чтобы главное было слева, а не в
      // случайном месте.
      const measured = d.semanticClusters
...
// Обход ГОТОВОЙ разметки по текстовым кускам (между «>» и «<»): внутрь тегов
// и атрибутов не заходим, иначе ссылка в href превратится в ссылку в ссылке.
// Сноски [1], [3] в тексте — переход к списку источников этого же модуля.
...
const ФРАЗЫ_РУ = {
  ...
  // Подзаголовки, которые модель печатает по-английски вопреки правилу языка.
  // Владелица 15.09 нашла три подряд в одном модуле.
  'top-3 priorities':'Три главных приоритета', 'top 3 priorities':'Три главных приоритета',
  'top priorities':'Главные приоритеты', 'what to defer':'Что откладываем',
  'what data/tests needed':'Какие данные и проверки нужны',
  'what data needed':'Какие данные нужны', 'what tests needed':'Какие проверки нужны',
...
const МЕТКИ_РУ = {
  ...
  'segment':'Сегмент', 'segments':'Сегменты', 'demand':'Спрос',
  'competition':'Конкуренция', 'economics':'Экономика', 'score':'Итог',
  'verdict':'Вердикт', 'priority':'Приоритет', 'niche':'Ниша', 'niches':'Ниши',
  'audience':'Аудитория', 'insight':'Вывод', 'insights':'Выводы',
  'recommendation':'Рекомендация', 'recommendations':'Рекомендации',
  'rationale':'Обоснование', 'confidence':'Уверенность', 'status':'Статус',
  'total':'Итого', 'notes':'Примечания', 'note':'Примечание',
...
function поРусски(html) {
  return внеСсылок(html, function (кусок) { return поТексту(кусок, function (t) {
    // Подпись целой строкой. Прежняя маска брала только буквы и пробелы,
    // поэтому «Top-3 priorities:» (цифра и дефис) и «What data/tests needed:»
    // (косая черта) мимо неё проходили — владелица 15.09 нашла их в модуле.
    const целиком = ФРАЗЫ_РУ[t.trim().toLowerCase().replace(/:\s*$/, '').replace(/\s+/g, ' ')];
    if (целиком) return целиком + (/:\s*$/.test(t) ? ':' : '');
...
// начале» — значит второй раз их печатать незачем; «аббревиатуры тут эти
// встречаются?» — TAM/SAM/SOM остались от выключенного модуля ёмкости и в
...
// Список аббревиатур в конце модуля владелица попросила убрать: «если нужно,
// то лучше это сделать в нужном месте справкой». Разбираем список в словарь,
// сам список выкидываем, а термины в тексте показываем подсказкой при
// наведении — объяснение оказывается там, где человек споткнулся, а не в
// хвосте, куда он не дойдёт.
// Где начинается и кончается список аббревиатур. Модель печатает его то
// заголовком «## Аббревиатуры», то обычной строкой «Аббревиатуры:» — на
// второй форме первая версия чистки промахнулась, и владелица 15.09 увидела
// список на месте: «всё равно внизу есть». Конец блока — заголовок, черта
// или конец текста.
// Расшифровки, напечатанные без заголовка: строкой под таблицей
// («*CR — коэффициент конверсии») или подряд списком после блока. Владелица
// 14.09 нашла их после гипотез и в мастерской офферов: «зачем эти пояснения
// здесь, если они у нас в тексте?» Форма всегда одна — короткий термин, тире
// или двоеточие, толкование. Толкование не пропадает: его подбирает
// собратьСловарь и вешает подсказкой у самого слова.
function строкаРасшифровки(строка) {
  const м = String(строка || '').match(
    /^\s*[-*•]?\s*\*{0,2}([A-ZА-ЯЁ][A-Za-zА-Яа-яЁё0-9/]{1,11})\*{0,2}\s*(?:[—–-]|:)\s+(\S[\s\S]{2,600})$/);
  if (!м) return null;
  // Не путать с предложением: «Мы — команда из трёх человек» тоже подходит
  // под форму. Термин обязан выглядеть сокращением: заглавные подряд, латиница
  // или косая черта внутри.
  const т = м[1];
  if (!/^[A-ZА-ЯЁ]{2,}(\/[A-ZА-ЯЁ]{2,})*$/.test(т) && !/^[A-Z][A-Za-z]*[A-Z]/.test(т)) return null;
  // В одной строке их бывает несколько подряд, через точку: «JTBD — … .
  // CJM — карта пути клиента. VoC — голос клиента.» Берём первое толкование
  // до начала следующего термина, остальные подберёт разбор всей строки.
  let толк = м[2].split(/(?=(?:^|[.;]\s+)[A-ZА-ЯЁ][A-Za-zА-Яа-яЁё0-9/]{1,11}\s*(?:[—–-]|:)\s)/)[0];
  толк = толк.replace(/\s*\([^)]*\)\s*$/, '').replace(/[.;]\s*$/, '').trim();
  return толк ? [т, толк] : null;
}

// Все расшифровки из одной строки, а не только первая: модель печатает их
// подряд («JTBD — … . CJM — … . VoC — …»), и владелица 14.09 показала такую
// строку в M7 — из неё раньше подбиралась одна.
function всеРасшифровки(строка) {
  const куски = String(строка || '')
    .split(/(?:^|(?<=[.;])\s+)(?=[A-ZА-ЯЁ][A-Za-zА-Яа-яЁё0-9/]{1,11}\s*(?:[—–-]|:)\s)/);
  const найдено = куски.map(к => строкаРасшифровки(к)).filter(Boolean);
  // Строка считается расшифровкой целиком, только если ею занято всё:
  // предложение с тире посередине под это не подходит.
  return найдено.length && найдено.length === куски.filter(к => к.trim()).length
    ? найдено : null;
}
...
function собратьСловарь(тело) {
  ...
  // Сначала одиночные строки-расшифровки: они встречаются и без списка.
  строки.forEach(строка => {
    (всеРасшифровки(строка) || []).forEach(п => {
      if (п[0] && п[1]) словарь[п[0]] = п[1];
    });
  });
  ...
  строки.slice(блок.начало + 1, блок.конец).forEach(строка => {
    ...
    // в подсказке это шум.
    const толк = п[2].replace(/\s*\([^)]*\)\s*$/, '').trim();
    if (термин && толк) словарь[термин] = толк;
  });
  return словарь;
}

function собратьВыводы(тело) {
  // Маркер ищем и по-английски: промпт просит «CONCLUSION (in <язык>):», и в
  ...
  for (let i = 0; i < строки.length; i++) {
    const м = строки[i].match(/^\s*\*{0,2}(?:Вывод|CONCLUSION|ИТОГ БЛОКА)\*{0,2}\s*:?\s*(.*)$/i);
    if (!м) continue;
...
function собратьШаги(тело) {
  const строки = String(тело || '').split('\n');
  const заголовокШагов =
    /^\s*#{0,4}\s*\*{0,2}\s*(?:итог\s*\/\s*следующие\s+шаги|следующие\s+шаги|что\s+делаем\s+дальше)\s*:?\s*\*{0,2}\s*:?\s*$/i;
  // Разделов «Итог / Следующие шаги» в модуле НЕ ОДИН: модель печатает свой
  // после каждого блока — в живом M3 их было пять. Брали только первый, и в
  // карточку попадали шаги блока источников («использовать подтверждённые
  // источники с отзывами»), а настоящие выводы про рынок и конкурентов не
  // доезжали. Владелица 14.09 назвала эту карточку сломанной — справедливо.
  const пункты = [];
  const видели = Object.create(null);
  // Шаги из блока источников — указания САМОМУ СЕБЕ: «использовать
  // подтверждённые источники», «расширить мониторинг отзывов». Заказчику они
  // не говорят ничего, а стоят первыми и вытесняют выводы про рынок. В живом
  // M3 карточка состояла из них целиком.
  const служебныйБлок = /источник|sources|выдержк/i;
  let блок = '';
  for (let i = 0; i < строки.length; i++) {
    const з = строки[i].match(/^#{1,4}\s*(.+?)\s*$/);
    if (з) блок = з[1];
    if (!заголовокШагов.test(строки[i])) continue;
    if (служебныйБлок.test(блок)) continue;
    for (let j = i + 1; j < строки.length; j++) {
      const х = строки[j];
      if (/^#{1,4}\s+\S/.test(х)) break;
      if (/^\s*\|/.test(х)) break;
      if (заголовокШагов.test(х)) break;
      // Конец списка — любая строка другого рода: имя следующего блока в
      // кавычках, подпись с двоеточием, жирный заголовок таблицы. Без этого
      // в шаги затекали заголовки соседних блоков.
      if (/^\s*[«"*]/.test(х) || /:\s*$/.test(х.trim())) break;
      const т = х.replace(/^\s*(?:[-*•]|\d+[.)])\s*/, '').trim();
      if (!т) continue;
      // Без номера источников и без хвостовой пунктуации — иначе один и тот
      // же шаг из двух блоков читается как два разных.
      const ключ = т.toLowerCase().replace(/\[\d+\]/g, '').replace(/[^а-яёa-z0-9]/g, '');
      if (!ключ || видели[ключ]) continue;
      видели[ключ] = true;
      пункты.push(т);
      i = j;
    }
  }
  // Карточка — сводка, а не пересказ: семь пунктов человек читает, двадцать
  // пролистывает.
  return пункты.slice(0, 7);
}

// Подписи, за которыми ничего нет. Содержимое SWOT уезжает в карточки, а
...
function убратьПустыеПодписи(тело) {
  const строки = String(тело || '').split('\n');
  const подпись = х => /^\s*\*{0,2}[А-ЯЁA-Z][^\n]{2,60}:\*{0,2}\s*$/.test(х);
  const заголовок = х => /^\s*#{1,4}\s+\S/.test(х);
  const содержимое = х => х.trim() && !подпись(х) && !заголовок(х);
  const вышло = [];
  for (let i = 0; i < строки.length; i++) {
    if (!подпись(строки[i])) { вышло.push(строки[i]); continue; }
    let j = i + 1, есть = false;
    while (j < строки.length && !подпись(строки[j]) && !заголовок(строки[j])) {
      if (содержимое(строки[j])) { есть = true; break; }
      j++;
    }
    if (есть) вышло.push(строки[i]);
  }
  return вышло.join('\n');
}
...
function почиститьХвост(тело, естьИтог) {
  ...
  t = t.replace(/\s*\((?:in|на)\s+Russian\)/gi, '');
  // Раздел = заголовок и всё до следующего заголовка того же или высшего уровня.
  const вырезать = условие => {
    const строки = t.split('\n');
    const вышло = [];
    let пропускаем = false, уровень = 0;
    for (const строка of строки) {
      const з = строка.match(/^(#{1,4})\s*(.+?)\s*$/);
      if (з) {
        const у = з[1].length;
        if (пропускаем && у <= уровень) пропускаем = false;
        if (!пропускаем && условие(з[2], у)) { пропускаем = true; уровень = у; continue; }
      }
      if (!пропускаем) вышло.push(строка);
    }
    t = вышло.join('\n');
  };
  if (естьИтог) {
    // Без \b: в JS он опирается на латиницу, и после кириллического «итог»
    // границы слова не находит — раздел «Итог / Следующие шаги» так и оставался.
    вырезать(имя => /^(итог|вывод|следующие\s+шаги)(\s|\/|:|$)/i.test(имя));
    // То же самое, но обычной строкой без решёток: модель печатает
    // «Итог / Следующие шаги:» и списком под ним. Такие строки заголовками не
    // считаются, и раздел оставался в тексте после каждого блока
    // (владелица 15.09 прислала модуль — там их пять штук).
    const строки3 = t.split('\n'), вышло3 = [];
    for (let i = 0; i < строки3.length; i++) {
      const голая = строки3[i].replace(/\*/g, '').trim();
      if (!/^(итог\s*\/\s*следующие\s+шаги|следующие\s+шаги|итог)\s*:?\s*$/i.test(голая)) {
        вышло3.push(строки3[i]); continue;
      }
      // Снимаем и сам заголовок, и относящийся к нему список до первой
      // строки другого рода: заголовка, таблицы или пустого абзаца подряд.
      let j = i + 1;
      while (j < строки3.length) {
        const х = строки3[j];
        if (/^\s*#{1,4}\s+\S/.test(х) || /^\s*\|/.test(х)) break;
        if (!х.trim() && j + 1 < строки3.length && !строки3[j + 1].trim()) break;
        j++;
      }
      i = j - 1;
    }
    t = вышло3.join('\n');
    // Тот же вывод модель печатает и внутри блоков — то отдельной строкой
    // «Вывод:», под которой через пустую строку идёт абзац, то сразу текстом.
    // Режем оба вида: сам маркер и относящийся к нему абзац.
    const строки2 = t.split('\n'), вышло2 = [];
    for (let i = 0; i < строки2.length; i++) {
      const м = строки2[i].match(/^\s*\*{0,2}(?:Вывод|CONCLUSION|ИТОГ БЛОКА)\*{0,2}\s*:?\s*(.*)$/i);
      if (!м) { вышло2.push(строки2[i]); continue; }
      if (м[1].trim()) continue;                    // вывод был на той же строке
      while (i + 1 < строки2.length && !строки2[i + 1].trim()) i++;   // пустые
      while (i + 1 < строки2.length && строки2[i + 1].trim()
             && !/^#{1,4}\s|^\|/.test(строки2[i + 1])) i++;          // сам абзац
    }
    t = вышло2.join('\n');
  }
  // Легенда уверенности, напечатанная самой моделью («5 = 3+ независимых
  // свежих источника; 4 = …»). Раньше промпт просил её печатать, теперь её
  // показывает интерфейс под нужной таблицей — а в старых прогонах строка
  // осталась и висела под источниками разведки (владелица 15.09).
  t = t.split('\n').filter(х =>
    !/(?:^|\s)5\s*=\s*3\+?\s*независим/i.test(х)
    && !/^\s*\*{0,2}(?:Confidence|Уверенность)\*{0,2}\s*(?:scale|шкала)?\s*:?\s*$/i.test(х)
  ).join('\n');

  // Список аббревиатур уходит целиком: его содержимое живёт подсказками у
  // самих терминов (см. собратьСловарь и подсказкиТерминов). Ищем его тем же
  // способом, что и разбор, — заголовком и обычной строкой с двоеточием.
  for (;;) {
    const строки3 = t.split('\n');
    const блок = найтиБлокАббревиатур(строки3);
    if (!блок) break;
    строки3.splice(блок.начало, блок.конец - блок.начало);
    t = строки3.join('\n');
  }
  // Расшифровки строкой, без заголовка-списка, — туда же, в подсказки.
  t = t.split('\n').filter(х => !всеРасшифровки(х)).join('\n');
  // Горизонтальные черты убираем совсем. Модель ставит «---» до и после
  // каждого блока, но у нас блоки и так разделены заголовками и карточками —
  // черта между ними ничего не отделяет, а после вырезанных разделов их
  // оставалось по две подряд. Владелица 15.09 показала это дважды.
  // Разделитель таблицы («|---|---|») под маску не подходит: он с палкой.
  // Формы черты: «---», «- - -», «***», «___», длинные тире и «═══» из
  // шаблонов промпта. Разделитель таблицы («|---|») сюда не попадает: он с палкой.
  t = t.split('\n').filter(х =>
    !/^\s*(?:[-*_—–]\s*){3,}$/.test(х) && !/^\s*[═=]{3,}\s*$/.test(х)).join('\n');
  t = убратьПустыеПодписи(t);
  return t.replace(/\n{3,}/g, '\n\n').trim();
}

// Часть таблиц строит скрипт уже в браузере (приоритет ниш, персоны, кольцо
// сегментов) — текстовые проходы по разметке их не видят, и в них оставались
// английские подписи: владелица 15.09 показала «Confidence» в таблицах
// приоритета. Проходим по готовым узлам после отрисовки. Работает и на сайте,
// и в выгруженном отчёте: словарь там тот же.
// Сноски [n] внутри того, что нарисовал скрипт (карточки ниш, персоны,
// таблицы приоритета). Текстовые проходы по разметке их не видят: этих узлов
// в разметке ещё нет. Владелица 15.09: «ссылки неактивные». Цель ищем на
// самой странице — блок источников уже отрисован и у него есть якорь.
function ссылкиВУзлах(корень) {
  if (!корень || typeof document === 'undefined') return;
  const блок = ['04_0', '02', '05'].map(k => 'блок-' + k)
  ...
  while ((узел = обход.nextNode())) {
    if (!узел.nodeValue || узел.nodeValue.indexOf('[') < 0) continue;
    if (!/\[\d{1,2}\]/.test(узел.nodeValue)) continue;
    if (узел.parentNode && узел.parentNode.closest && узел.parentNode.closest('a')) continue;
    цели.push(узел);
  }
  цели.forEach(узел => {
    const куски = узел.nodeValue.split(/(\[\d{1,2}\])/);
    const фрагмент = document.createDocumentFragment();
    куски.forEach(кусок => {
      const m = кусок.match(/^\[(\d{1,2})\]$/);
      if (!m) { фрагмент.appendChild(document.createTextNode(кусок)); return; }
      const a = document.createElement('a');
      a.className = 'ref'; a.href = '#' + блок;
      a.title = 'Показать источники разведки';
      a.textContent = кусок;
      фрагмент.appendChild(a);
    });
    узел.parentNode.replaceChild(фрагмент, узел);
  });
}

function почиститьУзлы(корень) {
  if (!корень || typeof document === 'undefined') return;
  const обход = document.createTreeWalker(корень, NodeFilter.SHOW_TEXT, null);
  const правки = [];
  let узел;
  while ((узел = обход.nextNode())) {
    ...
    if (!было || !/[A-Za-z]{3}/.test(было)) continue;
    // Внутри ссылок не трогаем: там адрес, а не подпись.
    if (узел.parentNode && узел.parentNode.closest && узел.parentNode.closest('a')) continue;
    const стало = поРусскиСтроку(было);
    if (стало !== было) правки.push([узел, стало]);
  }
  правки.forEach(([у, з]) => { у.nodeValue = з; });
}

// Перевод подписей в ГОЛОЙ строке (без разметки). Тот же словарь, что у
// поРусски, — иначе на сайте и в отчёте подписи разойдутся.
function поРусскиСтроку(t) {
  let s2 = String(t).replace(/\b[A-Za-z][A-Za-z ]{4,24}[A-Za-z]\b/g, фраза => {
    const русская = ФРАЗЫ_РУ[фраза.toLowerCase().replace(/\s+/g, ' ')];
    return русская || фраза;
  });
  return s2.replace(/\b([A-Za-z]{3,14})\b/g, слово => {
    const русское = МЕТКИ_РУ[слово.toLowerCase()];
    if (!русское) return слово;
    return слово[0] === слово[0].toLowerCase() ? русское.toLowerCase() : русское;
  });
...
// выгруженном файле отчёта, и при печати в PDF ничего не ломает.
function подсказкиТерминов(html, словарь) {
  // Модуль длинный, и одной пометки на весь модуль мало: владелица 14.09 —
  // «часть помечены, часть нет». Помечаем первое вхождение в КАЖДОМ блоке.
  const куски = String(html || '').split(/(?=<h2)/);
  if (куски.length > 1) return куски.map(к => подсказкиТерминов(к, словарь)).join('');
  return подсказкиВКуске(html, словарь);
}
function подсказкиВКуске(html, словарь) {
  const термины = Object.keys(словарь || {})
    .filter(т => т.length >= 2).sort((a, b) => b.length - a.length);
  if (!термины.length) return html;
  const экран = т => т.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Границы слова вручную: \b в JS опирается на латиницу и на кириллице врёт.
  const маска = new RegExp('(^|[^0-9A-Za-zА-Яа-яЁё])(' + термины.map(экран).join('|')
    + ')(?![0-9A-Za-zА-Яа-яЁё])', 'g');
  const уже = Object.create(null);
  return внеСсылок(html, кусок => поТексту(кусок, t => t.replace(маска, (весь, до, термин) => {
    // Подсказку вешаем на ПЕРВОЕ вхождение в модуле: на каждом слове подряд
    // пунктир превращается в рябь и мешает читать.
    if (уже[термин]) return весь;
    уже[термин] = true;
    return до + '<abbr class="term" title="' + escHtml(словарь[термин]) + '">' + термин + '</abbr>';
  })));
}

function оформитьТекст(html, блокИсточников, словарь) {
  ...
  // ссылка сломалась бы молча. Подсказки — последними, по готовому тексту.
  // <hr> убираем на выходе, какой бы формой он ни пришёл: у нас блоки
  // разделены заголовками и карточками, линия между ними — шум.
  // Владелица 15.09 показала их дважды, и в исходнике они встречаются
  // в разном написании.
  return подсказкиТерминов(
    именаБлоковВТексте(
      поРусски(поЧеловечески(сноскиНаИсточники(оживитьСсылки(html), блокИсточников)))),
    словарь).replace(/<hr\s*\/?>/gi, '');
}

// Шкала уверенности объясняется ТАМ, где встречается её колонка, а не общей
// строкой в начале модуля: владелица 15.09 сначала искала её под источниками,
// потом поняла, что оценка относится к нишам. Правильный ответ — сноска под
// той самой таблицей, какой бы она ни была. Цифры выделены, пояснения нет:
// глазами находишь свою цифру, а не читаешь строку целиком.
const ШКАЛА_УВЕРЕННОСТИ = [
  ['5', 'три и более независимых свежих источника'],
  ['4', 'два источника'],
  ['3', 'один источник'],
  ['2', 'косвенные данные или оценка'],
  ['1', 'публичных данных нет'],
];
const ШКАЛА_ДОСТАТКА = [
  ['H', 'высокая платёжеспособность'],
  ['M', 'средняя'],
  ['L', 'низкая'],
];
// Числа в сигналах ниши — жирным. Владелица 15.09: «где примерная экономика,
// цифры и тысяч рублей в месяц надо повыделять жирным». Выделяем только то,
// что действительно величина: сумму с единицей (₽, руб, тыс, млн, %) или
// число с разрядами (45 000). Одинокие цифры не трогаем — иначе жирными
// станут ссылки на источники [3] и оценки по пятибалльной шкале.
function выделитьЧисла(html) {
  return поТексту(html, function (t) {
    return t.replace(
      /(\d[\d\u00A0\s]{2,}\d|\d+(?:[.,]\d+)?(?:\s*[-–—]\s*\d+(?:[.,]\d+)?)?)((?:\s*(?:₽|руб\.?|рублей|тыс\.?|тысяч|млн|%))+)?/g,
      function (весь, число, единица) {
        const разряды = /[\u00A0\s]/.test(число);
        if (!единица && !разряды) return весь;
        return '<span class="strong">' + весь.trim() + '</span>';
      });
  });
}

function легендаУверенности(сДостатком) {
  // Каждая шкала — своей строкой и с двоеточием: в одну строку они читались
  // как продолжение друг друга (владелица 15.09).
  const ряд = (имя, шкала) => '<span class="cl-row"><span class="cl-h">' + имя + ':</span>'
    + шкала.map(function (п) {
        return '<span class="cl-i"><b>' + п[0] + '</b> — ' + п[1] + '</span>';
      }).join('') + '</span>';
  // Буквы H/M/L рядом с цифрами объясняем тут же: владелица 15.09 спросила,
  // что они значат, — значит, в таблице они стоят без пояснения.
  return '<p class="note confleg">' + ряд('Уверенность', ШКАЛА_УВЕРЕННОСТИ)
    + (сДостатком ? ряд('Достаток', ШКАЛА_ДОСТАТКА) : '') + '</p>';
}

function поТексту(html, как) {
  // Режем по тегам и правим только то, что между ними. Прежний вариант искал
  ...
  return String(html).split(/(<[^>]*>)/).map(function (часть) {
    return часть.charAt(0) === '<' ? часть : как(часть);
  }).join('');
}
...
// «BLOCK 09 показывает, что…» — так модель ссылается на соседний блок внутри
// фразы. Номер — наша внутренняя разметка; читателю нужно имя.
function именаБлоковВТексте(html) {
  return поТексту(html, t => t.replace(
    /\bBLOCK\s*([0-9]{1,2}[_0-9A-Z]*)\b/gi,
    (всё, код) => {
      const имя = BLOCK_TITLES[String(код).toUpperCase()];
      return имя ? '«' + имя + '»' : всё;
    }));
}
...
function оживитьКусок(html) {
  return поТексту(html, function (t) {
    return t.replace(/(^|[\s(«])((?:https?:\/\/|www\.)[^\s<>»)\]]+)/g, function (m, до, адрес) {
      const хвост = (адрес.match(/[.,;:!?]+$/) || [''])[0];
      const чистый = хвост ? адрес.slice(0, -хвост.length) : адрес;
      const href = /^https?:/i.test(чистый) ? чистый : 'https://' + чистый;
      const видно = чистый.replace(/^https?:\/\//i, '').replace(/\/$/, '');
      return до + '<a href="' + href + '" target="_blank" rel="noreferrer">'
        + (видно.length > 52 ? видно.slice(0, 50) + '…' : видно) + '</a>' + хвост;
    });
  });
}
...
function renderResearchHTML(content, opts) {
  ...
  function renderSegments(headers, rows) {
    ...
    // Кольцо и таблица стояли впритык, хотя место в строке есть: владелица
    // 15.09 — «нужен воздух между диаграммой и данными». 36px — тот же шаг,
    // что между колонками в других блоках эталона.
    return '<div class="seg" style="grid-template-columns:190px minmax(0,1fr);gap:36px">'
  ...
  function renderNiches(headers, rows, ctx) {
    ...
    // Итог — сумма осей, вердикт — по баллам, порядок — от большего к меньшему.
    // Владелица 15.09: «пусть идёт по порядочку от самого большого значения к
    // самому маленькому, и цвет как на карточках — до 15 рабочая ниша».
    // Раньше вердикт брался из колонки модели и спорил с её же оценкой,
    // а строки шли в том порядке, в каком модель их напечатала.
    D.forEach(д => {
      const сумма = д.v.reduce((a, b) => a + b, 0);
      if (сумма > 0) д.t = сумма;
      // Баллы задают цвет и порядок, НО прямое «не идём» от разведки сильнее:
      // только у таких ниш печатается разбор «почему не идём», и владелица
      // 15.09 просила их сохранить — «мы их проверили, и видно почему».
      // Порог снизу тоже оставляем: ниша с восемью баллами не «под вопросом».
      д.vd = д.vd === 'no' ? 'no'
        : (д.t >= ПОРОГ_БЕРЁМ ? 'go' : (д.t >= ПОРОГ_ВОПРОС ? 'mb' : 'no'));
    });
    // Сначала те, куда идём, потом спорные, в конце отброшенные; внутри — по
    // баллам от большего к меньшему.
    const вес = v => v === 'no' ? 2 : (v === 'mb' ? 1 : 0);
    D.sort((a, b) => (вес(a.vd) - вес(b.vd)) || ((b.t || 0) - (a.t || 0)));
    ...
    return '<div class="nwrap"><div>'
      ...
      // Где проходит граница — словами. Владелица 15.09: «должно быть
      // понимание, на какой градации мы уже не берём в работу». Цвет и
      // порядок карточек держатся ровно на этих числах.
      + '<p class="note confleg"><span class="cl-row">'
      + '<span class="cl-h">Как читаем оценку:</span>'
      + '<span class="cl-i"><b>15–20</b> — берём в работу</span>'
      + '<span class="cl-i"><b>10–14</b> — под вопросом</span>'
      + '<span class="cl-i"><b>ниже 10</b> — не идём</span>'
      + '</span><span class="cl-row"><span class="cl-i">Ниша с прямым отказом '
      + 'разведки остаётся внизу списка приглушённой — с разбором, почему не идём.'
      + '</span></span></p>';
  }
  ...
  function renderMarket(headers, rows, ctx, brief) {
    ...
    return '<div class="mktwrap"><div class="mkt" id="rpt-mkt"></div></div>'
      ...
      + (себяНашли
          ...
          : '<span>нас в этой таблице нет — модель не назвала нас среди игроков,'
            + ' поэтому <b>свой ценовой уровень не подсвечен</b></span>')
  ...
  function renderSources(headers, rows, ctx) {
    ...
    if (!проверяемых) {
      // Ни одной ссылки — доказывать нечем. Блок не печатаем вовсе.
      return '';
    }
    if (ctx && ctx.источникиМесто != null) {
      const былиАдреса = new Set((ctx.источникиСписок || []).map(x => x[4]));
      const добавка = d.filter(x => x[4] && !былиАдреса.has(x[4]));
      if (!добавка.length) return '';
      ctx.источникиСписок = (ctx.источникиСписок || []).concat(добавка)
        .map((x, i) => [i + 1, x[1], x[2], x[3], x[4]]);
      blockScripts[ctx.источникиМесто] =
        'renderSources(' + safeJson(ctx.источникиСписок) + ', 1);';
      const скольких = ctx.источникиСписок.filter(x => /^https?:\/\//i.test(String(x[4] || ''))).length;
      // Подпись свёртки обещает число ссылок — после слияния их больше.
      // Правим прямо в готовой разметке: она уже собрана выше по тексту.
      if (ctx.источникиЗаголовок) {
        blockScripts.push('(function(){var к=document.querySelector(".srcfold>.srctoggle");'
          + 'if(к) к.textContent="Источники: ' + скольких + ' '
          + plural(скольких, 'ссылка', 'ссылки', 'ссылок')
          + ', по которым собран этот модуль";})();');
      }
      return '';
    }
    const номерИст = ++ctxSrcN;
    ctx.источникиСписок = d.map((x, i) => [i + 1, x[1], x[2], x[3], x[4]]);
    blockScripts.push('renderSources('+safeJson(ctx.источникиСписок)+', '+номерИст+');');
    ctx.источникиМесто = blockScripts.length - 1;
    ctx.источникиЗаголовок = true;
  ...
  function renderGap(headers, rows) {
    ...
    const кандидаты = headers.filter(h => h !== kC && !skip.test(h));
    // Тепловая карта годится ТОЛЬКО для матрицы «признак × конкурент», где в
    // клетках короткие оценки. У гэп-анализа колонки бывают текстовые —
    // «Лучшее у конкурентов», «URL примера», «В чём гэп», — и они закрашивались
    // как оценки, а сам текст не показывался (скрин владелицы 15.09).
    // Считаем колонку оценочной, если почти все её значения — из словаря
    // уровней. Меньше двух таких колонок — рисуем обычную таблицу.
    const словарьУровней = /^(да|нет|есть|отсут[а-яё]*|win|parity|lose|полн[а-яё]*|сильн[а-яё]*|частич[а-яё]*|средн[а-яё]*|слаб[а-яё]*|—|-|н\/д)$/i;
    const оценочная = h => {
      const значения = rows.map(r => String(r[h] || '').trim()).filter(Boolean);
      if (!значения.length) return false;
      const похожих = значения.filter(v => словарьУровней.test(v.split(/[(,;·]/)[0].trim())).length;
  ...
  function renderPersonasBlock(headers, rows) {
    ...
    const D = rows.map((r,i) => {
      ...
      return {
        ...
        // Число, а не список: разметка печатает его как «N проверенных цитат».
        quotes: kQ ? (String(r[kQ]||'').trim() ? 1 : 0) : 0,
        // Третий столбец — «что из этого следует». В BLOCK 09 его нет, и
        // раньше на его месте печаталось «undefined» (владелица 14.09).
        rows: [
  ...
  function renderJourney(headers, rows) {
    ...
    if (kPers) {
      const поПерсонам = {};
      rows.forEach(r => {
        const п = String(r[kPers]||'').trim();
        (поПерсонам[п] = поПерсонам[п] || []).push(r);
      });
      const самая = Object.keys(поПерсонам)
        .sort((a, b) => поПерсонам[b].length - поПерсонам[a].length)[0];
      if (самая != null && поПерсонам[самая].length >= 3) строки = поПерсонам[самая];
    }
  ...
  function renderOfferInputBlock(headers, rows) {
    ...
    // Мало назвать колонку блока — человеку нужно поле брифа, куда это
    // писать (владелица 15.09). Отдельных полей под кейсы, гарантии и
    // ограничения в брифе нет: они живут в «Дополнительном контексте»,
    // цена — в «Ценовом диапазоне», гео — в «Географии работы».
    const кудаПисать = имя => {
    ...
    const поПолям = {};
    нет.forEach(имя => {
      const п = кудаПисать(имя);
      (поПолям[п] = поПолям[п] || []).push(имя);
    });
    const списком = Object.keys(поПолям).map(п =>
      '<li>в поле <b>«' + escHtml(п) + '»</b> — ' + поПолям[п].map(escHtml).join(', ') + '</li>').join('');
    const подпись = нет.length
      ? '<p class="note">Заполнено ' + (D.length - нет.length) + ' полей из ' + D.length
        + '. Не хватает: <b>' + нет.map(escHtml).join(', ') + '</b>.</p>'
        + '<p class="note">Что делать: вкладка <b>«Бриф»</b> → <b>«Править бриф»</b>, дальше:</p>'
        + '<ul class="note" style="margin:6px 0 0 18px">' + списком + '</ul>'
        + '<p class="note">Потом нажмите <b>↺</b> на этом модуле — офферы пересоберутся уже '
        + 'с этими данными. Пока поля пусты, модель не имеет права их придумать: выдуманная '
        + 'гарантия или кейс уйдут в рекламу, и отвечать по ним придётся по-настоящему.</p>'
      : '';
  ...
  function renderRadarChannelsBlock(headers, rows) {
    ...
    // выглядит неаккуратно». Разбираем по-честному: номер отдельно, адрес
    // отдельно.
    const разобратьСсылку = т => {
      const с = String(т || '').trim();
      const м = с.match(/\[([^\]]*)\]\(([^)]+)\)/);
      if (м) return { номер: (м[1].match(/\d{1,3}/) || [''])[0], адрес: м[2].trim() };
      const адрес = (с.match(/https?:\/\/\S+/) || [''])[0].replace(/[),.]+$/, '');
      return { номер: (с.replace(/https?:\/\/\S+/, '').match(/\d{1,3}/) || [''])[0], адрес };
    };
    // «Не замерено» дважды подряд («не замерено · не замерено») — это не
    // признак живости, а его отсутствие. Такие пометки не печатаем вовсе.
    const живое = т => {
      const в = String(т || '').trim();
      return (!в || в === '—' || /^(не\s+замерено|нет\s+данных|не\s+видно)/i.test(в)) ? '' : в;
    };
    const d = rows.map(r => {
      const n = g(r,kN);
      if (!n) return null;
      const площадка = g(r,kP);
      const живость = [живое(g(r,kSub)), живое(g(r,kF))].filter(Boolean).join(' · ');
      const что = [живое(g(r,kWho)), живое(g(r,kFmt))].filter(Boolean).join(' · ');
      const ссылка = разобратьСсылку(kU ? r[kU] : '');
      return [ n, площадка, значок(площадка), что, живость, '', ссылка.номер, ссылка.адрес ];
    }).filter(Boolean);
    if (!d.length) return null;
    blockScripts.push('renderChannels('+safeJson(d)+');');
    return '<div class="chn" id="rpt-chn"></div>';
  ...
  function renderShareBlock(headers, rows) {
    ...
    const d = rows.map(r => {
      ...
      const v = numOf(r[kV]);
      if (!n) return null;
      return [n, v == null ? 0 : v, kA ? String(r[kA]||'').trim() : ''];
    }).filter(Boolean).sort((a, b) => b[1] - a[1]);
    if (!d.length) return null;
    // Кольцо, а не полосы: доли складываются в целое (владелица 15.09 — «тут
    // же из ста, можно полный круг»). Полосы сравнивают величины между собой,
    // кольцо показывает, как одно целое поделено — здесь верно второе.
    blockScripts.push('renderShareRing('+safeJson(d)+');');
    return '<div class="seg" style="grid-template-columns:230px minmax(0,1fr);gap:36px">'
      + '<div id="rpt-share"></div><div id="rpt-share-leg"></div></div>';
  }
  ...
  function renderTopContentBlock(headers, rows) {
    ...
    const d = rows.map(r => {
      ...
      return { t, v: v == null ? 0 : v, raw: g(r,kV) || 'не замерено',
               p: g(r,kP), ic: опознатьПлощадку(g(r,kP) + ' ' + t), resp: g(r,kR),
  ...
  // Последняя попытка перед сырой таблицей: разбор «в нашем стиле» для любой
  // таблицы-перечня — имя строки слева, содержимое под ним. Владелица 15.09
  // прошла по М3 и назвала шесть таких мест подряд: «куда расти», «главное
  // обещание», «три главных приоритета», «что отложить», «какие данные нужны»,
  // «ориентиры рынка». У каждой свой заголовок, который модель придумывает
  // заново, — гоняться за ними по именам бессмысленно.
  //
  // Не трогаем то, что действительно таблица: где есть числовые колонки, их
  // сравнивают глазами по столбцу, и карточки этому мешают.
  function renderGenericCards(headers, rows) {
    // До восьми колонок: шире шести строка в отчёте всё равно не читается, и
    // карточка честнее прокрутки. Выше — это уже выгрузка, не чтение.
    if (headers.length < 2 || rows.length < 2) return null;
    const g = (r, k) => String(r[k] == null ? '' : r[k]).replace(/\*\*/g, '').trim();
    const число = v => /^[\d\s\u00A0.,%+-]+$/.test(String(v || '').trim()) && /\d/.test(String(v || ''));
    const числовая = h => rows.filter(r => число(g(r, h))).length >= rows.length * 0.6;
    // Первая колонка бывает просто номером («1, 2, 3») — это не данные, а
    ...
    let столбцы = headers;
    if (числовая(headers[0]) && headers.length > 2
        && rows.every(r => g(r, headers[0]).length <= 3)) столбцы = headers.slice(1);
    if (столбцы.slice(1).some(числовая)) return null;
    // Имя строки должно быть именем, а не абзацем.
    const имена = rows.map(r => g(r, столбцы[0])).filter(Boolean);
    if (имена.length < 2) return null;
    const средняя = имена.reduce((а, x) => а + x.length, 0) / имена.length;
    if (средняя > 70) return null;
    const D = rows.map(r => {
      const имя = g(r, столбцы[0]);
      if (!имя || имя === '—') return null;
      const поля = столбцы.slice(1).map(h => [h, g(r, h)])
        .filter(x => x[1] && x[1] !== '—' && !/^не\s+(видно|замерено|задано)/i.test(x[1]));
      if (!поля.length) return null;
      return [имя, поля];
    }).filter(Boolean);
    if (D.length < 2) return null;
    blockScripts.push('renderListCards(' + safeJson(D) + ');');
    return '<div class="rules" id="rpt-generic-' + (++ctxGenericN) + '"></div>';
  }
  let ctxGenericN = 0;
...
// Отступ между абзацами — не содержимое. Нужен при проверке «под заголовком
// что-то есть»: без этого пустая строка делала пустой заголовок «полным».
const безОтступов = h => String(h).replace(/(<div style="height:8px"><\/div>)+$/, '');
function mdToHtml(text) {
    ...
    const ctx = swotCtx;
    ctx.h1seen = false;
    // Якорь по номеру блока: по нему сноски [n] уводят к «Источникам
    // разведки» (владелица 15.09: «чтобы они переключались на ссылки в
    // источниках, а дальше человек перейдёт на конкретную ссылку»).
    const якорь = t => {
      const m = String(t || '').match(/^(?:BLOCK\s*([0-9]{1,2}[_0-9A-Z]*)|(SEO-\d\d))\b/i);
      return m ? ' id="блок-' + (m[1] || m[2] || '').toUpperCase() + '"' : '';
    };
    const flushTable = () => {
      ...
      // Шкала уверенности встаёт под «Приоритетом ниш»: оценка относится к
      // нишам, а не к сегментам и не к источникам (владелица 15.09 нашла её
      // под сегментами — туда она попадала просто потому, что колонка
      // «Confidence» там встречается раньше по тексту). Если матрицы ниш в
      // модуле нет вовсе — показываем под первой таблицей, где есть колонка:
      // без неё цифры в столбце остаются без объяснения.
      // Шкала стоит под ТОЙ таблицей, где реально есть колонка: владелица
      // 15.09 сначала искала её у ниш, потом нашла колонку у сегментов —
      // значит, угадывать место не нужно, надо привязываться к колонке.
      const естьКолонка = hdrs.some(h => /уверенн|confidence/i.test(h));
      ...
      const пусто = v => {
        ...
        if (!t) return true;
        if (/^(—|-|н\/д|нет данных|нет публичных данных|не замерено|не задано|не применимо|не определено|n\/a|none)\s*[.,;)]?\s*$/i.test(t)) return true;
        // Развёрнутые формы того же самого — их диктует промпт: «нужны данные
        // рекламного кабинета или Метрики заказчика», «не видно из источника».
        return /^(нужны\s+данные|нужен\s+доступ|требуется\s+доступ|не\s+видно\s+из\s+источник|нет\s+в\s+открыт|публичных\s+данных\s+нет|данных\s+нет)/i.test(t);
      };
      const колонкиДанных = hdrs.length > 1 ? hdrs.slice(1) : hdrs;
      const естьДанные = asObjs.some(o => колонкиДанных.some(h => !пусто(o[h])));
      // Демография прячется ЦЕЛИКОМ, вместе с заголовком и вводным абзацем
      ...
      if (естьДанные && /BLOCK\s*09B\b|Демограф/i.test(lastHeading)) ctx.демографияЕсть = true;
      if (!естьДанные && /BLOCK\s*09B\b|Демограф/i.test(lastHeading)
          && ctx.headStart != null) {
      ...
      if (/BLOCK\s*04_1\b/i.test(lastHeading)) {
        ...
        if (kn) for (const r of asObjs) {
          ...
          ctx.signals[nm] = hdrs.filter(h => h !== kn && String(r[h]||'').trim())
            .map(h => [h.replace(/\s*\(.*\)/,''), выделитьЧисла(mdInline(String(r[h])))]);
        }
      }
      ...
      ctx.htmlLen = html.length; ctx.dropHead = false; ctx.заголовок = lastHeading;
      // Метка надёжнее имени: имя модель переписывает, метку — нет.
      const поМетке = ctx.метка ? 'BLOCK ' + ctx.метка : '';
      ...
      const карточками = renderGenericCards(hdrs, asObjs);
      if (карточками) { html += карточками + сноска(); tableRows=[]; inTable=false; return; }
    ...
    for (const line of lines) {
      ...
      if (/^#{1,4}\s/.test(line)) {
        ...
        if (!четверть) lastHeading = имяЗ;   // сырой — по нему узнаём формат
               ...
               && t.length <= 60
               && !/[.;]\s|\sчто\s|\sпоказыва|\sсвязыва|\sотнос/i.test(t)) {
      ...
      // Служебный блок данных для графиков. Он вход для рисовалок, а не текст
      // для заказчика: в отчёте это абзац про «проценты являются гипотезами».
      if (/^#{1,4}\s*(данные\s+для\s+визуализац|chart\s*data)/i.test(line)) {
        ctx.служебныйБлок = true; continue;
      }
      if (ctx.служебныйБлок) {
        if (/^#{1,4}\s+\S/.test(line)) ctx.служебныйБлок = false;
        else continue;
      }
      ...
      if (уровень) {
        ...
        // Пустая строка печатает отступ — он не содержимое. Иначе любой
        // заголовок с пустой строкой под ним считался бы заполненным.
        while (ctx.пустые && ctx.пустые.length
               && ctx.пустые[ctx.пустые.length - 1].у >= у
               && безОтступов(html).length <= ctx.пустые[ctx.пустые.length - 1].конец) {
          html = html.slice(0, ctx.пустые.pop().начало);
        }
        if (ctx.пустые) ctx.пустые = ctx.пустые.filter(x => x.у < у);
      }
      if (line.startsWith('# ')) {
        ...
        ctx.headStart = html.length;
        // Демография сегмента прячется целиком: и пустой таблицей, и одной
        // фразой «блок не применим». Запоминаем начало и решаем на выходе.
        if (ctx.демографияС != null && !ctx.демографияЕсть) {
          html = html.slice(0, ctx.демографияС);
        }
        ...
        let имя3 = human(line.slice(4)).trim();
        // «### Таблица 2. Главное обещание» шло мимо чистки: ловили только
        // жирную строку и обычный абзац (владелица 15.09).
        if (/^(таблица|табл\.)\s*\d+\s*[—.:-]/i.test(имя3)) {
          имя3 = сБольшой(имя3.replace(/^(таблица|табл\.)\s*\d*\s*[—.:-]?\s*/i, '')
            .replace(/[:.]\s*$/, '').trim());
        }
        if (!(ctx.has062 && /^(сильные\s+стороны|слабые\s+стороны|возможности|угрозы)\s*:?$/i.test(имя3))) {
          const н3 = html.length;
          html += '<h3'+якорь(line.slice(4))+'>'+esc(имя3)+'</h3>';
          (ctx.пустые = ctx.пустые || []).push({ у: 3, начало: н3, конец: html.length });
        }
      }
      else if (line.startsWith('#### ')) {
        const н4 = html.length;
        html += '<h4'+якорь(line.slice(5))+'>'+esc(human(line.slice(5)))+'</h4>';
        (ctx.пустые = ctx.пустые || []).push({ у: 4, начало: н4, конец: html.length });
      }
      // Модель часто ставит звёздочки вкривь: «*Таблица 1. Карта рынка:**».
      ...
      // строку, обрамлённую звёздочками с обеих сторон, сколько бы их ни было.
      else if (/^\*{1,2}[^*].*[^*]\*{1,2}$/.test(t) && t.replace(/\*/g, '').trim()) {
        // Модель печатает подзаголовки жирным абзацем: «**Таблица 2. Наш
        // ценовой уровень**», «**Легенда:**», «**Комментарий:**». Владелица
        // 15.09: «служебный жирный текст посреди данных». Разводим по смыслу:
        // название таблицы — подзаголовок, пояснение — приписка, остальное
        // остаётся жирной строкой, как было.
        ...
        if (/^(таблица|табл\.)\s*\d*/i.test(имя)) {
          ...
          // «Комментарий:») заказчику ничего не дают: он смотрит на данные,
          // а не на то, как модель их для себя размечала. Подпись снимаем —
          // текст под ней остаётся, если в нём есть смысл.
          continue;
        } else {
          html += '<p><b>' + esc(имя) + '</b></p>';
        }
      }
      else if (t.startsWith('**') && t.includes('**')) {
        // Та же служебная подпись, но не одна в строке, а в начале абзаца:
        // «**Легенда:** С — сильные стороны». Строкой целиком её уже снимали,
        // а эту форму пропускали — владелица 15.09 нашла её снова.
        const голова = (t.match(/^\*{1,2}\s*([^*:]{2,20}):?\s*\*{1,2}/) || [])[1] || '';
        if (/^(легенда|определение|как читать|расшифровка)/i.test(голова.trim())) continue;
        if (/^(комментарий|примечание|пояснение)/i.test(голова.trim())) {
          // Подпись снимаем, текст оставляем: в нём бывает настоящее
          // наблюдение, а не разметка модели для себя.
          const хвост = t.replace(/^\*{1,2}\s*[^*:]{2,20}:?\s*\*{1,2}\s*/, '').trim();
          if (хвост) html += '<p>'+mdInline(хвост)+'</p>';
          continue;
        }
        html += '<p>'+mdInline(t)+'</p>';
      }
      // Маркер списка рисуем мы; если модель ещё и пронумеровала пункт внутри,
      // выходит «• 1. Текст» — маркер и число разом (владелица 15.09).
      // Номер убираем, порядок и так виден.
      // «Таблица 1 — главная ставка:» обычной строкой, без звёздочек.
      // Нумерация таблиц — рабочая пометка из промпта, заказчику она не
      // говорит ничего (владелица просила убрать трижды).
      else if (/^(таблица|табл\.)\s*\d+\s*[—:.-]/i.test(t)) {
        const хвост = t.replace(/^(таблица|табл\.)\s*\d*\s*[—.:-]?\s*/i, '')
          .replace(/[:.]\s*$/, '').trim();
        if (хвост) html += '<h3>' + esc(сБольшой(хвост)) + '</h3>';
      }
      else if (/^[-•·]\s/.test(t)) html += '<p class="rli">'+mdInline(t.slice(2).replace(/^\s*\d+[.)]\s+/, ''))+'</p>';
      else if (/^\d+\.\s/.test(t)) html += '<p class="rli">'+mdInline(t.replace(/^\s*\d+[.)]\s+/, ''))+'</p>';
    ...
    if (ctx.демографияС != null && !ctx.демографияЕсть) {
      html = html.slice(0, ctx.демографияС);
      ctx.демографияС = null;
    }
    // Последний заголовок тоже может остаться без содержимого.
    while (ctx.пустые && ctx.пустые.length
           && безОтступов(html).length <= ctx.пустые[ctx.пустые.length - 1].конец) {
      html = html.slice(0, ctx.пустые.pop().начало);
    }
    return безОтступов(html);
  }


  const сырой = mdToHtml(content);
  // Куда ведут сноски: у разведки это «Источники разведки» (04_0), у остальных
  ...
  const html = оформитьТекст(сырой, блокИсточников, (opts && opts.словарь) || null);
...
function renderModuleSummary(s) {
  ...
  const part = (title, items, cls) => items && items.length
    ? '<div class="' + cls + '"><span class="smh">' + esc(title) + '</span><ul>'
      + items.map(x => '<li>' + mdInlineSafe(сБольшой(x)) + '</li>').join('') + '</ul></div>'
    : '';
...
function generateHTMLReport(brief, results, lang, priceLayers, selectedLayers, singleModId) {
  ...
  targetResults.filter(r=>r.id==='M8').forEach((rep, i) => {
    ...
    const nicheSub = rep.niche ? 'Ниша: '+rep.niche+' · ' : '';
    const withVolume = semCl.filter(c => c.volume != null && c.volume > 0);
    if (withVolume.length) {
      pushReportChart(resKey(rep), canvasId, 'Семантические кластеры по объёму', nicheSub+'Только кластеры с частотностью, подтверждённой источником.', 220);
      extraScripts += 'if(document.getElementById("'+canvasId+'")){var ss='+safeJson(withVolume)+';'
        +'new Chart(document.getElementById("'+canvasId+'"),{type:"bar",data:{labels:ss.map(function(c){return c.cluster}),datasets:[{data:ss.map(function(c){return c.volume}),backgroundColor:"#72243E"}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}}}});}';
    } else {
      // Частотность нигде не подтверждена источником — рисовать объём было бы её выдумкой.
      // Вместо этого честный график: сколько кластеров на каждый приоритет (1-3) — это
      // пересказ таблицы SEO-02, не оценка спроса, поэтому не требует подтверждения [n].
      pushReportChart(resKey(rep), canvasId, 'Семантические кластеры по приоритету', nicheSub+'Частотность источниками не подтверждена (нужен ручной замер в Wordstat) — вместо объёма показано число кластеров по приоритету.', 200);
      extraScripts += 'if(document.getElementById("'+canvasId+'")){var ss='+safeJson(semCl)+';'
        +'var byP={1:0,2:0,3:0};ss.forEach(function(c){var p=c.priority;if(byP[p]!=null)byP[p]++;});'
        +'new Chart(document.getElementById("'+canvasId+'"),{type:"bar",data:{labels:["Приоритет 1","Приоритет 2","Приоритет 3"],datasets:[{data:[byP[1],byP[2],byP[3]],backgroundColor:["#72243E","#A8506E","#D896AC"]}]},options:{indexAxis:"y",responsive:true,plugins:{legend:{display:false}},scales:{x:{beginAtZero:true,ticks:{precision:0}}}}});}';
    }
  });
  ...
  const html = '<!DOCTYPE html>\n<html lang="ru">\n<head>\n<meta charset="UTF-8">\n'
    ...
    +'\n/* Нечего мерить — нет и дорожки: две колонки вместо трёх. */'
    +'\n.nomeasure .tcrow{grid-template-columns:minmax(0,1fr) auto}'
    +'\n.topc .tcrow:first-child{border-top:0}'
    +'\n.tcn{font-size:13.5px;line-height:1.35;color:var(--ink);min-width:0}'
    +'\n.tcn .tci{display:inline-block;width:15px;height:15px;vertical-align:-2px;margin-right:6px}'
    +'\n.tcn .tci svg{width:15px;height:15px;stroke:var(--ink-3);fill:none;stroke-width:1.6}'
    ...
    +'\n/* Последняя строка широкой карточки — служебная приписка «у соперников\n   занято / рискуем», а не цитата. Серифом 19px она выглядела как главная\n   мысль карточки и спорила с такой же строкой в соседней узкой карточке,\n   набранной мелким серым (скрин владелицы 15.09). Приводим к одному виду. */\n.bet .rule-card.wide>span:last-child{max-width:52ch}'
    ...
    // У гэп-анализа шесть колонок, у карты рынка восемь. На экране
    ...
    + (blockScripts.length
        ? '<script>'+blockJS+'\nwindow.addEventListener("load",function(){'
          ...
          // Подписи в таблицах, нарисованных скриптом, — по-русски. Словарь
          // тот же, что на сайте: иначе отчёт и страница разойдутся в словах.
          // Функции переносим ИМЕНОВАННО: анонимное выражение не создаёт имя,
          // и обход упал бы на первом же вызове поРусскиСтроку.
          + '\ntry{var ФРАЗЫ_РУ='+safeJson(ФРАЗЫ_РУ)+',МЕТКИ_РУ='+safeJson(МЕТКИ_РУ)+';'
          + 'var поРусскиСтроку='+поРусскиСтроку.toString()+';'
          + 'var почиститьУзлы='+почиститьУзлы.toString()+';'
          + 'var ссылкиВУзлах='+ссылкиВУзлах.toString()+';'
          + 'почиститьУзлы(document.body);ссылкиВУзлах(document.body);}catch(e){}'
          + '});<'+'/script>'
        : '')
    +'\n</body>\n</html>';
...
// Круговая на четыре сектора — по решению владелицы 15.09: «паутинка не очень
// хороша, а если круговую на четыре сектора и у каждого своя заливка — было бы
// наглядно». Радиус сектора = оценка (1–5), то есть площадь читается как мера,
// а не как украшение. Заливка — оттенки ОДНОГО цвета (цвет вердикта ниши):
// величина передаётся светлотой, поэтому сектора различимы и при дальтонизме,
// а identity держат подписи осей, не цвет.
const ОСИ_НИШИ = [
  ['demand', 'Спрос', 'сколько людей ищут решение'],
  ['competition', 'Конкуренция', 'насколько занято место'],
...
function РозаНиши({ ниша, размер }) {
  const S = размер || 190, C = S / 2, R = S / 2 - 16, MAX = 5;
  const значение = k => {
    const v = Number(ниша[k]);
    return Number.isFinite(v) ? Math.max(0, Math.min(MAX, v)) : 0;
  };
  const цвет = цветВердикта(ниша.verdict);
  // Оттенки держим одной семьёй: величину несёт радиус, а слишком светлый
  // сектор читается как «мало», хотя оценка там высокая (скрин 15.09).
  const тон = [100, 88, 76, 64];
  // Сектор: дуга от угла к углу с радиусом по оценке. Углы считаем от 12 часов
  // по часовой стрелке, чтобы порядок совпадал с порядком строк в табличке.
  const сектор = (i, r) => {
    const a0 = (-90 + i * 90) * Math.PI / 180, a1 = (-90 + (i + 1) * 90) * Math.PI / 180;
    const x0 = C + r * Math.cos(a0), y0 = C + r * Math.sin(a0);
    const x1 = C + r * Math.cos(a1), y1 = C + r * Math.sin(a1);
    return 'M ' + C + ' ' + C + ' L ' + x0.toFixed(1) + ' ' + y0.toFixed(1)
      + ' A ' + r.toFixed(1) + ' ' + r.toFixed(1) + ' 0 0 1 ' + x1.toFixed(1) + ' ' + y1.toFixed(1) + ' Z';
  };
  return (
    <svg viewBox={'0 0 ' + S + ' ' + S} width="100%" height={S} role="img"
      aria-label={'Оценки ниши ' + (ниша.name || '')}>
      {[1, 2, 3, 4, 5].map(k => (
        <circle key={k} cx={C} cy={C} r={R * k / MAX} fill="none"
          stroke={k === MAX ? 'var(--line)' : 'var(--line-2)'} strokeWidth="1"/>
      ))}
      {ОСИ_НИШИ.map(([k], i) => {
        const v = значение(k);
        if (!v) return null;
        return <path key={k} d={сектор(i, R * v / MAX)}
          fill={'color-mix(in srgb, ' + цвет + ' ' + тон[i] + '%, transparent)'}
          stroke="var(--card-solid)" strokeWidth="2"/>;
      })}
      {[0, 1, 2, 3].map(i => {
...
function NicheHero({ list, canPick, selected, onToggle, onContinue, statusOf, шапка, наПерламутре, onPick }) {
  ...
  return (
    <div className={наПерламутре ? 'cover' : 'card'}
      ...
      <div className="dock">
        {/* Круговая на четыре сектора и разбор рядом: владелице не хватало
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(0,190px) minmax(0,1fr)',
          ...
          <РозаНиши ниша={тек}/>
          <div>
            {ОСИ_НИШИ.map(([k, имя, пояснение]) => {
              const v = Number(тек[k]);
              const есть = Number.isFinite(v);
              return (
                <div key={k} style={{display:'grid',gridTemplateColumns:'minmax(0,136px) minmax(0,1fr) 34px',
                    gap:12,alignItems:'center',marginBottom:13}}>
                  <div>
                    <div style={{fontSize:12.5,fontWeight:600,lineHeight:1.2}}>{имя}</div>
                    <div style={{fontSize:11,color:'var(--ink-3)',lineHeight:1.35,marginTop:2}}>{пояснение}</div>
                  </div>
                  <span style={{display:'block',height:14,background:'var(--line-2)'}}>
                    <i style={{display:'block',height:14,width:(есть ? Math.max(2, v / 5 * 100) : 0) + '%',
                      background:цветВердикта(тек.verdict)}}/>
                  </span>
                  <span style={{fontSize:12.5,fontWeight:700,textAlign:'right',
                    fontVariantNumeric:'tabular-nums'}}>{есть ? v : '—'}</span>
                </div>
              );
            })}
            <div style={{display:'flex',alignItems:'baseline',gap:10,paddingTop:11,
              ...
              <span style={{fontSize:11,fontWeight:700,letterSpacing:'.07em',
                textTransform:'uppercase',color:'var(--ink-3)'}}>Итог</span>
              <b style={{fontSize:22,letterSpacing:'-.02em'}}>{тек.score != null ? тек.score : '—'}</b>
              <span style={{fontSize:11.5,color:'var(--ink-3)'}}>из 20 баллов</span>
              {тек.verdict && (
                <span className={'kchip ' + (/no|не идём|нет/i.test(тек.verdict) ? 'kchip-no'
                  : (/maybe|вопрос/i.test(тек.verdict) ? 'kchip-mb' : 'kchip-go'))}
                  style={{marginLeft:'auto'}}><span className="d"></span>{тек.verdict}</span>
              )}
            </div>
          </div>
        </div>
...
function StageHeader({ имя, подпись, раздел, факты, lang }) {
  ...
  //     <span class="kchip kchip-go rpt-mark"><span class="d"></span>Раздел</span>
  //     <span class="eyebrow">бровь</span>
  //     <h1 class="covername">Имя экрана</h1>
  //     <p class="coversub">Одна фраза</p>
  //   </div></div><dl class="coverdl">…факты…</dl></div>
  // Своего оформления здесь нет намеренно: владелица просила «вынести плашку
  // ПО ОБРАЗЦУ», и образец — вот этот.
  //
  // .rview стоит на ОБЁРТКЕ, а не на самой карточке: правила эталона
    ...
    <div className="rview">
      <div className="card nacre cover" style={{marginBottom:22}}>
        ...
        <dl className="coverdl">
          ...
          <div><dt>Язык исследования</dt><dd>{langSelf(lang)}</dd></div>
          <div><dt>Дата</dt><dd>{дата}</dd></div>
        </dl>
      </div>
    </div>
  );
...
// Перенос стилей эталона в область .rview. Вынесено из injectBlockStyles
// отдельными функциями, чтобы это можно было ПРОВЕРИТЬ тестом: ошибка здесь
// не падает и не логируется — она молча выбрасывает правила, и страница
// выглядит черновиком (14.09: «оформлено уродски, сплошной текст»).
function rviewScope(sel) {
...
function rviewPrefix(css) {
  // КОММЕНТАРИИ СНИМАЕМ ДО разбора, иначе они прилипают к селектору: правило
  // с пояснением над ним превращалось в «.rview /* текст */ … .dtbl{…}»,
  // браузер такой селектор не понимает и ВЫБРАСЫВАЕТ правило целиком. Так
  // молча пропадало оформление .dtbl, .kbtn, .sech и всех, у кого над
  // правилом стоит пояснение, — а пояснения тут почти над каждым.
  // В base64-картинке «/*» встретиться не может: звёздочки нет в алфавите.
  //
  // Начало правила — начало строки, ИЛИ закрывающая скобка, ИЛИ открывающая
  // скобка @media/@supports. Без третьего случая первое правило внутри
  // каждого @media оставалось без ограничения и утекало на всю страницу.
  return css
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/(^|[{}])\s*([^{}@]+)\{/g, (m, br, sel) => br + '\n' + rviewScope(sel) + '{');
}

function injectBlockStyles() {
  if (blockStyleInjected || typeof document === 'undefined') return;
  blockStyleInjected = true;
  const prefix = rviewPrefix;
...
function ResearchView({ content, ourName, ourPrice, выбранные, наВыбор }) {
  ...
  React.useEffect(() => {
    ...
    // КАЖДЫЙ блок в своей ловушке. Раньше все рисовалки шли одним куском под
    // общим try: стоило одной споткнуться, и ВСЕ следующие блоки оставались
    // пустыми — владелица 14.09 видела «источники: 7 ссылок», а внутри
    // ничего. В выгруженном отчёте так и сделано, на сайте не было.
    try {
      const рисовать = new Function('__блоки', out.js + '\n' +
        'for (var i=0;i<__блоки.length;i++){ try { eval(__блоки[i]); }'
        + ' catch(e){ if(window.console) console.error("Блок не собрался:", e); } }');
      рисовать(out.scripts);
    ...
    try {
      ref.current.querySelectorAll('[id^="rpt-"]').forEach(узел => {
        ...
        if (/^rpt-srcs-/.test(узел.id)) return;   // ящик источников чистим отдельно
  ...
  // Кнопка «взять в работу» в карточках ниш рисуется скриптом и своего
  ...
  React.useEffect(() => {
    ...
    document.addEventListener('ca-nishe', слушатель);
    return () => document.removeEventListener('ca-nishe', слушатель);
  }, [наВыбор]);
  React.useEffect(() => {
    if (!ref.current) return;
    const взято = выбранные || [];
    ref.current.querySelectorAll('.ntake').forEach(кнопка => {
      const имя = кнопка.getAttribute('data-niche') || '';
      const есть = взято.some(х => String(х).toLowerCase() === имя.toLowerCase());
      кнопка.textContent = есть ? '✓ В работе' : '+ Взять в работу';
      кнопка.classList.toggle('on', есть);
    });
  }, [out, выбранные]);
...
// ── MAIN APP
// ── LOCK SCREEN (вход — Б1, Supabase Auth вместо общего пароля, 24.08.2026)
// Регистрация пока ручная (владелица заводит аккаунты в Supabase Dashboard) —
// самообслуживаемая форма регистрации это Б9, отдельный заход позже.
function LockScreen({ t, onUnlock }) {
  ...
  const submit = async e => {
    e.preventDefault();
    if (!email.trim() || !pw.trim() || busy) return;
    setBusy(true); setErr(false);
    const ok = await signIn(email.trim(), pw.trim()).catch(()=>false);
    setBusy(false);
...
function App() {
  ...
  // рефреш успешен только если сохранённый refresh_token ещё действителен.
  const [unlocked, setUnlocked] = React.useState(null);
  React.useEffect(() => {
    if (!getRefreshToken()) { setUnlocked(false); return; }
    refreshTokens().then(ok => setUnlocked(ok)).catch(() => setUnlocked(false));
  }, []);
  ...
  // Полный отчёт разведки на экране ниш свёрнут: решение по нишам принимают
  // по картам и разбору, а блоки источников и сегментов нужны не всем и не
  ...
  // Цена зависит от модели, которая реально пойдёт в работу: у каждого модуля
  ...
  const [addingPub, setAddingPub] = React.useState(false);
  const [metricsFormFor, setMetricsFormFor] = React.useState(null);
  const [metricsForm, setMetricsForm] = React.useState({});
  const [addingMetrics, setAddingMetrics] = React.useState(false);
  React.useEffect(() => {
    if (sc === 'tracker' && proj?.id) loadPublications(proj.id).then(setPublications);
  }, [sc, proj?.id]);
  // M8 — тренд-монитор (по проекту и нише)
  const [trendNiche, setTrendNiche] = React.useState('');
  const [trendSnapshots, setTrendSnapshots] = React.useState(null); // null = загрузка/БД недоступна
  const [generatingTrend, setGeneratingTrend] = React.useState(false);
  React.useEffect(() => {
    if (sc === 'trends' && proj?.id) loadTrendSnapshots(proj.id, trendNiche).then(setTrendSnapshots);
  }, [sc, proj?.id, trendNiche]);
  ...
  // Подтверждение сидов Wordstat/Google Ads перед M7 (26.08.2026) — та же
  // пауза-в-цепочке, что showNiches у M1.2. confirmedSeedsRef — не useState:
  // это только для чтения внутри run(), рендерить не нужно.
  const [showSeedConfirm, setShowSeedConfirm] = React.useState(false);
  const [seedCandidates, setSeedCandidates] = React.useState([]);
  const [seedError, setSeedError] = React.useState(null);   // подбор фраз не удался — показываем причину
  const [pendingM7Niche, setPendingM7Niche] = React.useState('');
  const confirmedSeedsRef = React.useRef({});
  const pausedRunRef = React.useRef(null); // параметры run(), прерванного паузой — см. continueAfterSeeds
  ...
  const run = React.useCallback(async (modsToRun, briefOverride, forceRerun, rerunNiche, regenNote) => {
    ...
    if (blockedMods.length) {
      setBlockMsg('Исследование не начато. ' + blockedMods.join('; ') + '. Эти модули строятся на данных предыдущих стадий: отметьте недостающие модули в списке или перегенерируйте цепочку кнопкой ↺ на более раннем модуле.');
      return;
    }
    setBlockMsg('');
    ...
    const col = [...existing];
    // Ниши, где какой-то модуль упал: остальные её модули пропускаем (см. ниже).
    const failedNiches = new Set();

    for (const w of runnable) {
      const mod = w.mod, wn = w.niche;
      if (failedNiches.has(wn)) continue; // предыдущий модуль этой ниши не выполнился
      // Бриф, сфокусированный на одной нише: фильтр ниш и запросы поиска сужаются до неё.
      // researchLang кладём в бриф, чтобы сборщики поисковых запросов знали язык
      // исследования: до 31.08 все запросы были захардкожены по-русски, и для
      ...
      if (mod.id === 'M8' && !confirmedSeedsRef.current[wn]) {
        ...
        setSeedCandidates(candidates.length ? candidates : ['']);
        setPendingM7Niche(wn);
        // Запоминаем, ЧЕМ был этот прогон, чтобы continueAfterSeeds продолжил его
        // теми же параметрами: иначе после паузы терялись rerunNiche (в пересчёт
        // уходили лишние ниши) и regenNote (замечание владелицы «что поправить»
        // молча пропадало).
        pausedRunRef.current = { modsToRun: allSelected, briefOverride: B, forceRerun, rerunNiche, regenNote };
        setShowSeedConfirm(true);
        return; // весь run() останавливается здесь — ждём continueAfterSeeds
      }

      // Re-run с замечанием: только ПЕРВЫЙ модуль запрошенной цепочки получает правку —
      ...
      try {
        ...
        if (mod.id === 'M2') {
          ...
          // ведёт себя как раньше, честное «не замерено». Сиды уже подтверждены
          // владелицей выше по циклу (см. паузу перед M7) — сюда доходим только
          // когда confirmedSeedsRef.current[wn] уже заполнен.
          ...
          // до 300 адресов, больше не влезет в промпт с пользой.
          const ownMap = Bn.siteUrl ? await callSitemap(Bn.siteUrl) : null;
          const rivalMaps = [];
          for (const d of (compDomains || []).slice(0, 2)) {
      ...
      // снаружи как «отработал, но ничего не нашёл»: 26.08 M3 упал с «Error: API
      ...
      // Дальше по цепочке этой ниши идти НЕЛЬЗЯ: зависимые модули построятся на
      // строке «Error: ...» вместо исследования и будут стоить денег впустую.
      // Так 26.08 M4 сделал три персоны, приняв за контекст падение M3. Другие
      // ниши не трогаем — у них свои данные, они не пострадали.
      if (failed) {
        failedNiches.add(wn);
        setBlockMsg('Модуль ' + (mod.label || mod.id) + (wn ? ' [' + wn + ']' : '') + ' не выполнился: ' + cleanedContent.trim()
          + '. Остальные модули этой ниши пропущены, чтобы не строить их на пустом результате — нажмите ↺ на этом модуле, когда будете готовы повторить.');
        continue;
      }
  ...
  const continueAfterSeeds = React.useCallback(() => {
    const confirmed = seedCandidates.map(s => s.trim()).filter(Boolean);
    if (!confirmed.length) return;
    confirmedSeedsRef.current[pendingM7Niche] = confirmed;
    setShowSeedConfirm(false);
    // Продолжаем ТОТ ЖЕ прогон, теми же параметрами (см. pausedRunRef) — run()
    // сам пропустит уже готовые (модуль,ниша) и дойдёт до M7 этой ниши, где
    // confirmedSeedsRef уже заполнен, поэтому повторной паузы не будет.
    const pr = pausedRunRef.current;
    pausedRunRef.current = null;
    // forceRerun НЕ переносим — намеренно. Он выкидывает из результатов всю
    // запрошенную цепочку, а к моменту паузы модули до M7 (M2…M6 при ↺ на M2)
    // уже посчитаны и оплачены — повтор forceRerun стёр бы их и прогнал заново
    // за деньги. Ниша и замечание переносятся: без rerunNiche в пересчёт ушли бы
    // соседние ниши.
    if (pr) run(pr.modsToRun, pr.briefOverride, false, pr.rerunNiche, pr.regenNote);
    else run(mods, brief);
  }, [seedCandidates, pendingM7Niche, mods, brief, run]);
  ...
  const genRep = React.useCallback(async () => {
    ...
    try {
      // Потолок вывода зависит от числа ниш: с двумя нишами отчёт теперь содержит
      // отдельный раздел на каждую плюс сводное сравнение, и в стандартные 8000
      // токенов не влезал — обрывался на середине последней ниши (тот же случай,
      // что уже был у M1.2, где потолок пришлось поднимать до 16000).
      const nCnt = nichesOf(brief).length || 1;
      const repTok = nCnt > 1 ? Math.min(8000 + 6000 * (nCnt - 1), 24000) : 8000;
  ...
  const buildFullMd = () => {
    ...
    // Тот же порядок и разделитель «Ниша: …», что в generateHTMLReport (иначе секции
    // по нескольким нишам идут вперемешку в порядке computed, без подписи чья секция).
    const modOrderIdx = MODULES.map(m => m.id);
    const ordered = dropOrphans(proj?.results||[]).sort((a, b) => {
      const ga = !a.niche, gb = !b.niche;
      if (ga !== gb) return ga ? -1 : 1;
      if ((a.niche||'') !== (b.niche||'')) return (a.niche||'') < (b.niche||'') ? -1 : 1;
      return modOrderIdx.indexOf(a.id) - modOrderIdx.indexOf(b.id);
    ...
    const parts = ordered.map(r=>{
      ...
      let prefix = '';
      if (r.niche && r.niche !== lastNiche && nichesOf(brief).length > 1) prefix = '# Ниша: '+r.niche+'\n\n';
      lastNiche = r.niche;
      return prefix+'## '+title+'\n\n'+r.content;
    });
  ...
  // Финальная проверка языка перед выгрузкой (латиница в русском тексте).
  // Считается по ВСЕМУ материалу — модули + резюме, — но только когда он изменился:
  // buildFullMd() склеивает сотни килобайт, гонять по ним регулярки на каждую
  // перерисовку незачем. Хук на верхнем уровне, а не в JSX: порядок хуков должен
  // быть постоянным, а панель живёт внутри условной ветки.
  const lint = React.useMemo(
    () => (proj?.results?.length ? lintForeignInsertions(buildFullMd(), lang) : { hard: [], soft: [], skipped: true }),
    [proj, rep, lang]
  );
  ...
  // ── HEADER
  // signOut — очищает токены и возвращает на экран входа. Нужна была для
  // переключения между аккаунтами при тестировании Б1-Б3 (25.08): без неё
  ...
  const Header = () => embedded ? null : (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        ...
        <button onClick={signOut} style={{padding:'5px 12px',fontSize:12,fontWeight:500}}>{t.signOut}</button>
      </div>
    </div>
  );
  ...
  if (viewReport) {
    // Старые прогоны (до перенумерации) в отчёт не идут: их модули под новыми
    // номерами читаются как каша. Показываем первый СВЕЖИЙ проект — или
    // честное «отчёта пока нет».
    const свежие = (projs || []).filter(p2 =>
      !(p2.results || []).some(r => r && r.id === 'M1_2'));
    const п = (proj && !(proj.results || []).some(r => r && r.id === 'M1_2') ? proj : null)
      || свежие[0] || null;
    ...
    if (!готов) return (
      <div className="card" style={{margin:'1.5rem'}}>
        ...
        <p style={{fontSize:13,color:'var(--ink-2)'}}>Он появится после первого исследования:
  ...
  // форма на всех. Владелица: «на Брифе всё подряд, то же самое на Нишах и
  ...
  if (embedded && !proj && (шагИзАдреса === 'niches' || шагИзАдреса === 'run')) return (
    <div>
      ...
      <div className="card">
        <h2>{шагИзАдреса === 'niches' ? 'Ниш пока нет' : 'Исследование ещё не запускалось'}</h2>
  ...
  if (embedded && sc === 'form' && proj && !briefEdit) {
    // Страница собрана ПО ОБРАЗЦУ согласованного макета (артефакт 5a34980c):
    // плашка-заголовок → карточки с <h2> → таблица эталона (.nm на главном
    // столбце) → приписка .note → строка действий с пояснением цены.
    // Своих раскладок здесь нет: три захода «по мотивам» владелица забраковала.
    const зн = v => String(v == null ? '' : v).trim();
    const домен = зн(brief.siteUrl).replace(/^https?:\/\/(www\.)?/i, '').replace(/\/$/, '');
    const ответы = [
      ['Что продаёт', brief.niche, 'ниша и продукт — с этого начинается разведка'],
      ['Кто покупатель', brief.audience, 'по нему собираются персоны и голос клиента'],
      ['Что обещаем клиенту', brief.result, 'главный результат — основа офферов'],
      ['Формат продукта', brief.format, ''],
      ['Цены и тарифы', brief.price, 'нужны, чтобы сравнивать конкурентов по своему уровню'],
      ['Известные конкуренты', brief.competitors, 'стартовый список; разведка добавит своих'],
      ['Что ещё важно знать', brief.extra, ''],
    ].filter(р => зн(р[1]));
    const цвета = зн(brief.brandColors).match(/#[0-9a-fA-F]{3,8}/g) || [];
    const соц = зн(brief.socials).split(/[\n,\s]+/).filter(x => /^https?:/.test(x));
    const шрифты = зн(brief.brandFonts);
        ...
        <div className="worksurface rview">
          <div className="card">
            ...
            <div className="tbl">
              <table>
                <tbody>
                  <tr><th>Поле</th><th>Что мы знаем</th></tr>
                  {ответы.map(([к, в, поясн]) => (
                    <tr key={к}>
                      <td style={{width:230}}>
                        <span className="nm">{к}</span>
                        {поясн && <><br/><span className="note">{поясн}</span></>}
                      </td>
                      <td>{зн(в)}</td>
                    </tr>
                  ))}
                  {зн(brief.geoCompany) && (
                    <tr><td style={{width:230}}><span className="nm">География компании</span></td>
                      <td>{зн(brief.geoCompany)}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            <p className="note" style={{marginTop:12}}>
              Собрано с сайта и из ваших ответов. Правка здесь стоит ноль, правка
              после прогона — повторный прогон модулей, которые на ней держатся.
            </p>
          </div>

          <div className="card">
            <h2>Дизайн и каналы клиента</h2>
            {естьДизайн ? (
              <React.Fragment>
                <div className="tbl">
                  <table>
                    <tbody>
                      <tr><th>Что взяли с сайта</th><th>Значение</th></tr>
                      {зн(brief.brandLogo) && (
                        <tr><td style={{width:230}}><span className="nm">Знак</span></td>
                          <td><img src={зн(brief.brandLogo)} alt="Логотип клиента"
                            style={{maxHeight:38,maxWidth:190,display:'block'}}
                            onError={e=>{ e.target.style.display='none'; }}/></td></tr>
                      )}
                      {цвета.length > 0 && (
                        <tr><td style={{width:230}}><span className="nm">Палитра</span></td>
                          <td><span style={{display:'flex',gap:16,flexWrap:'wrap',alignItems:'center'}}>
                            {цвета.map(c => (
                              <span key={c} style={{display:'inline-flex',alignItems:'center',gap:7}}>
                                <span style={{width:20,height:20,borderRadius:7,background:c,
                                  border:'1px solid var(--line)',
                                  boxShadow:'inset 0 1px 0 rgba(255,255,255,.45)',
                                  display:'inline-block',flexShrink:0}}/>
                                <span className="strong" style={{fontVariantNumeric:'tabular-nums'}}>{c}</span>
                              </span>
                            ))}
                          </span></td></tr>
                      )}
                      {шрифты && (
                        <tr><td style={{width:230}}><span className="nm">Шрифты</span></td>
                          <td>{шрифты}</td></tr>
                      )}
                      {соц.length > 0 && (
                        <tr><td style={{width:230}}>
                          <span className="nm">Свои площадки</span><br/>
                          <span className="note">их ритм и тон меряет контент-радар</span>
                        </td>
                          <td><span style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                            {соц.map(u => (
                              <a key={u} className="kchip" href={u} target="_blank" rel="noreferrer"
                ...
                <p className="note" style={{marginTop:12}}>
                  Уходит в контент-машину как есть — она покажет это на проверку,
                  а не спросит заново.
                </p>
              </React.Fragment>
            ) : (
              <p className="note" style={{marginTop:4}}>
                С сайта ничего не разобралось — это законно: сайта может не быть
                или цвета заданы картинками. Палитру, шрифты и свои площадки
                можно вписать руками кнопкой «Править бриф».
              </p>
            )}
          </div>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:'12px 18px',alignItems:'center',
            ...
            <p className="note" style={{margin:0,maxWidth:'52ch'}}>
              Нашли ошибку — поправьте до запуска модулей: пересчёт стоит денег,
  ...
  if (sc === 'tracker') return (
    <div>
      ...
      {publications === null ? (
        ...
        <React.Fragment>
          <div className="card">
            <p style={{fontSize:13,fontWeight:500,marginBottom:10}}>+ Добавить публикацию</p>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
              <Field label="Дата"><input type="date" value={pubForm.date} onChange={e=>setPubForm(p=>({...p,date:e.target.value}))}/></Field>
              <Field label="Площадка">
                <select value={pubForm.platform} onChange={e=>setPubForm(p=>({...p,platform:e.target.value}))}>
                  <option value="">—</option>
                  {['youtube_shorts','vk_clips','telegram','instagram_reels','instagram_carousel','dzen','rutube','max','pinterest','tiktok'].map(v=><option key={v} value={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Формат">
                <select value={pubForm.format} onChange={e=>setPubForm(p=>({...p,format:e.target.value}))}>
                  <option value="">—</option>
                  {['short_video','long_video','carousel','text_post'].map(v=><option key={v} value={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Персонаж">
                <select value={pubForm.character} onChange={e=>setPubForm(p=>({...p,character:e.target.value}))}>
                  {['rabbit','owner','none'].map(v=><option key={v} value={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Тема"><input value={pubForm.topic} onChange={e=>setPubForm(p=>({...p,topic:e.target.value}))} placeholder="3 ошибки в попапах"/></Field>
              <Field label="Гипотеза" optional><input value={pubForm.hypothesis_id} onChange={e=>setPubForm(p=>({...p,hypothesis_id:e.target.value}))} placeholder="H-03"/></Field>
              <Field label="Тип хука" optional><input value={pubForm.hook_type} onChange={e=>setPubForm(p=>({...p,hook_type:e.target.value}))} placeholder="вопрос / шок-факт"/></Field>
              <Field label="URL" optional><input value={pubForm.url} onChange={e=>setPubForm(p=>({...p,url:e.target.value}))} placeholder="https://..."/></Field>
              <Field label="UTM" optional><input value={pubForm.utm} onChange={e=>setPubForm(p=>({...p,utm:e.target.value}))} placeholder="utm_campaign=..."/></Field>
            </div>
            <button className="btn-primary" disabled={addingPub || !pubForm.topic.trim()}
              onClick={async ()=>{
                setAddingPub(true);
                const created = await addPublication({project_id:proj.id, ...pubForm});
                if (created) setPublications(p=>[{...created, metrics_snapshots:[]}, ...(p||[])]);
                setPubForm({date:new Date().toISOString().slice(0,10),platform:'',format:'',character:'none',topic:'',hypothesis_id:'',hook_type:'',url:'',utm:''});
                setAddingPub(false);
              }}
              style={{marginTop:4}}>
              {addingPub ? 'Добавляю…' : '+ Добавить публикацию'}
            </button>
          </div>
            ...
            {publications.map(pub => (
              <div key={pub.id} style={{border:'1px solid var(--line)',borderRadius:10,padding:'10px 14px',marginBottom:8}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}>
                  ...
                  <button onClick={()=>{setMetricsFormFor(metricsFormFor===pub.id?null:pub.id); setMetricsForm({snapshot_date:new Date().toISOString().slice(0,10)});}}
                  ...
                  <div style={{marginTop:8,overflowX:'auto'}}>
                    <table style={{fontSize:11,width:'100%',borderCollapse:'collapse'}}>
                      <thead><tr style={{color:'var(--ink-3)'}}>
                        ...
                        <th style={{padding:'2px 6px'}}>Досмотр %</th><th style={{padding:'2px 6px'}}>+Подписки</th>
                        <th style={{padding:'2px 6px'}}>Клики</th><th style={{padding:'2px 6px'}}>Регистрации</th>
                      </tr></thead>
                      <tbody>
                        {pub.metrics_snapshots.map((s,i)=>(
                          <tr key={i}>
                            <td style={{padding:'2px 6px'}}>{s.snapshot_date}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.views??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.likes??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.comments??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.shares??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.watch_pct??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.subs_gained??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.clicks??'—'}</td>
                            <td style={{padding:'2px 6px',textAlign:'center'}}>{s.registrations??'—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {metricsFormFor === pub.id && (
                  <div style={{marginTop:10,padding:10,background:'var(--card-solid)',borderRadius:8}}>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,marginBottom:8}}>
                      ...
                      <Field label="Лайки" optional><input type="number" value={metricsForm.likes||''} onChange={e=>setMetricsForm(f=>({...f,likes:e.target.value}))}/></Field>
                      <Field label="Комменты" optional><input type="number" value={metricsForm.comments||''} onChange={e=>setMetricsForm(f=>({...f,comments:e.target.value}))}/></Field>
                      <Field label="Репосты" optional><input type="number" value={metricsForm.shares||''} onChange={e=>setMetricsForm(f=>({...f,shares:e.target.value}))}/></Field>
                      <Field label="Досмотр %" optional><input type="number" value={metricsForm.watch_pct||''} onChange={e=>setMetricsForm(f=>({...f,watch_pct:e.target.value}))}/></Field>
                      <Field label="+Подписки" optional><input type="number" value={metricsForm.subs_gained||''} onChange={e=>setMetricsForm(f=>({...f,subs_gained:e.target.value}))}/></Field>
                      <Field label="Клики" optional><input type="number" value={metricsForm.clicks||''} onChange={e=>setMetricsForm(f=>({...f,clicks:e.target.value}))}/></Field>
                      <Field label="Регистрации" optional><input type="number" value={metricsForm.registrations||''} onChange={e=>setMetricsForm(f=>({...f,registrations:e.target.value}))}/></Field>
                    </div>
                    <button className="btn-primary" disabled={addingMetrics}
                      onClick={async ()=>{
                        setAddingMetrics(true);
                        const ok = await addMetricsSnapshot({publication_id:pub.id, ...metricsForm});
                        if (ok) {
                          const fresh = await loadPublications(proj.id);
                          if (fresh) setPublications(fresh);
                          setMetricsFormFor(null);
                        }
                        setAddingMetrics(false);
                      }}
                      style={{fontSize:12,padding:'6px 14px'}}>
                      {addingMetrics ? 'Сохраняю…' : 'Сохранить метрики'}
                    </button>
                  </div>
                )}
              </div>
          ...
          <button className="btn-primary" disabled={generatingTrend}
            onClick={async ()=>{
              ...
              try {
                ...
                const created = await addTrendSnapshot({
                  project_id: proj.id, niche: trendNiche,
                  snapshot_date: new Date().toISOString().slice(0,10),
                  trends: chartData.trends||[], newsworthy: chartData.newsworthy||[], delta: chartData.delta||{},
                  content: cleaned, usage: lastGptUsage, search_calls: searchCallCount,
                });
                if (created) setTrendSnapshots(s=>[created, ...(s||[])]);
              } catch(e) {}
  ...
  if (embedded && proj && шагИзАдреса === 'niches') {
    ...
    const изТаблицы = (() => {
      ...
      return (т.rows || []).map(r => ({
        ...
        // Прямое «не идём» от разведки сильнее баллов — как и в отчёте ниже.
        отказ: /не идём|no-?go/i.test(String(ключ(r, 'вердикт') || '')),
      })).filter(x => x.name);
    })();
    ...
    // ИТОГО считаем СУММОЙ четырёх осей, когда все четыре есть: на скрине
    // 15.09 карта показывала 15, а полосы под ней 4+4+5+5=18 — модель
    // ошиблась в собственной арифметике. Сумму человек может проверить
    // глазами, а число из колонки — нет.
    const сОценкой = н => {
      // Number(null) === 0, поэтому пустую ось нельзя проверять через Number:
      // иначе пропуск молча посчитается нулём и итог занизится.
      const оси = ['demand', 'competition', 'economics', 'fit']
        .map(k => (н[k] === null || н[k] === undefined || н[k] === '') ? NaN : Number(н[k]));
      const всеЕсть = оси.every(v => Number.isFinite(v));
      return { ...н, score: всеЕсть ? оси.reduce((a, b) => a + b, 0) : н.score };
    };
    const всеНиши = [...поИмени.values()].map(сОценкой)
      .map(н => ({ ...н, verdict: н.отказ ? 'не идём' : вердиктПоБаллам(н.score) }))
          ...
          <React.Fragment>
            ...
            <div className="worksurface rview">
              ...
              {/* Остальные блоки разведки: источники, сегменты, эффективность
                  ...
                  вкладке «Ход исследования» вперемешку с другими модулями. Место им
                  здесь: это один модуль — разведка ниш. */}
              {/* Запуск сразу под веером: решение принимают по картам и разбору,
                  а полный отчёт разведки убран под стрелку ниже (владелица
                  15.09). Сколько ниш и почём — видно на кнопке, до траты. */}
              <div style={{display:'flex',flexWrap:'wrap',gap:'12px 18px',alignItems:'center',
              ...
              {/* Полный отчёт разведки — под стрелкой. Решение по нишам
                  принимают по картам и разбору выше; источники, сегменты и
                  матрица приоритета нужны не всем и не всегда, а развёрнутые
                  они топили собой кнопку запуска. */}
              {м2 ? (
                <div style={{marginTop:26,paddingTop:20,borderTop:'1px solid var(--line)'}}>
                  <div style={{display:'flex',gap:14,alignItems:'baseline',flexWrap:'wrap'}}>
                    <button className="cm-btn" aria-expanded={отчётРазвёрнут}
                ...
                <div className="card" style={{marginTop:26}}>
                  ...
                  <p style={{fontSize:13.5,color:'var(--ink-2)',lineHeight:1.6,margin:0,maxWidth:'70ch'}}>
                    ...
                    вкладке «Ход исследования», чтобы получить источники и сегменты.
                  </p>
                </div>
              )}
            </div>
  ...
  return (
    <div>
      ...
      {embedded && showNiches && nicheOpts.length > 0 && (
        <div className="card" style={{marginBottom:16}}>
          <h2>Исследование ждёт выбора ниш</h2>
          <p style={{fontSize:13.5,color:'var(--ink-2)',lineHeight:1.6,maxWidth:'70ch',margin:0}}>
      ...
      {embedded && !isRun && (pending.length > 0 || doneCount > 0) && (
        <div style={{display:'flex',flexWrap:'wrap',gap:'12px 18px',alignItems:'center',
          ...
          <p className="note" style={{margin:0,maxWidth:'52ch'}}>
            {pending.length > 0
              ? 'Оценка считается по расходу прошлых запусков и умножается на число ниш. Точная сумма станет известна после.'
          ...
          <div style={{display:'flex',gap:9,flexWrap:'wrap',alignItems:'center'}}>
            {pending.length > 0 && (
              <button className="cm-btn cm-btn-pri" onClick={()=>run()}>
                Исследовать: {pending.map(m=>m.id).join(', ')}{(() => {
      ...
      {orderedResults.filter(r => !(embedded && r.id === 'M2')).map((r,i,arr) => {
        ...
        return (
          <React.Fragment key={resKey(r)}>
              ...
              <div style={{display:'flex',alignItems:'center'}}>
                ...
                {!isRun && (
                  <div style={{display:'flex',gap:5,margin:'0 8px',flexShrink:0}}>
                    <button onClick={e=>{
                        e.stopPropagation();
                        // Re-run с замечанием (human-in-the-loop): вместо мгновенной слепой
                        // перегенерации открываем панель — можно указать, что именно исправить,
                        // или оставить пустым для обычного поведения (как раньше).
                        setRegenKey(resKey(r)); setRegenNote('');
                      }}
                    ...
                    <button onClick={e=>{
                        ...
                        setEditKey(resKey(r)); setEditDraft(r.content); setExp(ex=>({...ex,[resKey(r)]:true}));
                      }}
                      style={{padding:'3px 8px',fontSize:10,color:m.dark,borderColor:m.border,background:'rgba(255,255,255,0.7)'}}
                      title="Редактировать результат вручную">
                      ✎
                    </button>
            ...
            {r.id === 'M6' && (r.niche||'') === pendingM7Niche && !isRun && showSeedConfirm && (
              <div style={{marginBottom:16}}>
                <div className="card">
                  ...
                  <div style={{display:'flex',gap:10}}>
                    <button className="btn-primary" onClick={continueAfterSeeds}
                      disabled={!seedCandidates.some(s=>s.trim())}>{t.seedContinue}</button>
                  </div>
                </div>
              </div>
            )}
