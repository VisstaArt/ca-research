...
function почиститьХвост(тело, естьИтог) {
  ...
  // Аббревиатуры: оставляем только те, что реально встречаются в тексте.
  t = t.replace(/(^#{1,4}\s*Аббревиатуры[^\n]*\n)([\s\S]*?)(?=\n#{1,4}\s|\s*$)/gim,
    (весь, шапка, список) => {
      const остальное = t.replace(весь, ' ');
      const строки = список.split('\n').filter(х => х.trim());
      const нужные = строки.filter(х => {
        const m = х.match(/([A-ZА-Я][A-Za-zА-Яа-я]{1,6})\s*[—-]/);
        if (!m) return false;
        return new RegExp('\\b' + m[1] + '\\b').test(остальное);
      });
      return нужные.length ? шапка + нужные.join('\n') + '\n' : '';
    });
  return t.replace(/\n{3,}/g, '\n\n').trim();
}
