...
function почиститьХвост(тело, естьИтог) {
  ...
  // Осиротевшие разделители. Вырезая разделы, мы оставляем «---» без того,
  // что они разделяли: владелица 15.09 показала два подряд — «лишние
  // артефакты». Убираем черту, если до неё ничего нет, если перед ней уже
  // была черта или если после неё до конца пусто.
  const разделитель = х => /^\s*(-{3,}|\*{3,}|_{3,})\s*$/.test(х);
  const значимая = х => х.trim() && !разделитель(х);
  const было = t.split('\n'), стало = [];
  let последняяЗначимая = -1;
  for (let i = 0; i < было.length; i++) {
    if (разделитель(было[i])) {
      if (последняяЗначимая < 0) continue;                       // черта в начале
      if (стало.length && разделитель(стало[стало.length - 1])) continue; // две подряд
      if (!было.slice(i + 1).some(значимая)) continue;           // черта в конце
      // Между чертой и предыдущей строкой могли остаться пустые — не помеха.
      стало.push('---');
      continue;
    }
    if (значимая(было[i])) последняяЗначимая = i;
    стало.push(было[i]);
  }
  t = стало.join('\n');
  return t.replace(/\n{3,}/g, '\n\n').trim();
    ...
    <div className={наПерламутре ? 'cover' : 'card'}
      ...
      <div className="dock">
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(0,190px) minmax(0,1fr)',
            gap:22,alignItems:'center',marginBottom:16}} className="ниша-разбор">
          <РозаНиши ниша={тек}/>
          <div>
