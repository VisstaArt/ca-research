...
function легендаУверенности() {
  return '<p class="note confleg"><span class="cl-h">Уверенность</span>'
    + ШКАЛА_УВЕРЕННОСТИ.map(function (п) {
...
function mdToHtml(text) {
    ...
    const flushTable = () => {
      ...
      // Есть колонка уверенности — под этой таблицей встанет её шкала. Один
      // раз на модуль: вторая такая же сноска ничего не добавляет.
      const сУверенностью = !ctx.легендаДана
        && hdrs.some(h => /уверенн|confidence/i.test(h));
      if (сУверенностью) ctx.легендаДана = true;
      const сноска = () => сУверенностью ? легендаУверенности() : '';
