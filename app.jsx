...
function строкаРасшифровки(строка) {
  const м = String(строка || '').match(
    /^\s*[-*•]?\s*\*{0,2}([A-ZА-ЯЁ][A-Za-zА-Яа-яЁё0-9/]{1,11})\*{0,2}\s*(?:[—–-]|:)\s+(\S.{2,200})$/);
  if (!м) return null;
  ...
  return [т, м[2].replace(/\s*\([^)]*\)\s*$/, '').replace(/[.;]\s*$/, '').trim()];
}
...
function собратьСловарь(тело) {
  ...
  строки.forEach(строка => {
    const п = строкаРасшифровки(строка);
    if (п && п[0] && п[1]) словарь[п[0]] = п[1];
  });
...
function почиститьХвост(тело, естьИтог) {
  ...
  t = t.split('\n').filter(х => !строкаРасшифровки(х)).join('\n');
